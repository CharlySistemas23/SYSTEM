...
// ELIMINADO77',message:'createDemoUsers ERROR',data:{error:error.message,stack:error.stack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            ...
            setTimeout(async () => {
// ELIMINADO83',message:'Auto-create usuarios timeout ejecutado',data:{dbReady:!!DB.db,createDemoUsersAvailable:typeof window.createDemoUsers==='function'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                try {
                    const users = await DB.getAll('users') || [];
// ELIMINADO86',message:'Auto-create usuarios check',data:{usersCount:Array.isArray(users)?users.length:'NOT_ARRAY',isArray:Array.isArray(users)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
...
// ELIMINADO90',message:'Auto-create usuarios ejecutado',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                            console.log('✅ Usuarios demo creados automáticamente');
                        } else {
// ELIMINADO92',message:'createDemoUsers NO DISPONIBLE',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
...
// ELIMINADO98',message:'Auto-create usuarios ERROR',data:{error:error.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
...
// ELIMINADO103',message:'bypassLogin llamado',data:{dbReady:!!DB.db},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                console.log('=== BYPASS LOGIN ===');
                try {
                    // Ensure database is ready
                    if (!DB.db) {
// ELIMINADO107',message:'bypassLogin DB no listo, esperando',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
...
// ELIMINADO1087',message:'Rule saved',data:{id:rule.id,agency:rule.agency_id,min:rule.min_passengers,max:rule.max_passengers},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                } catch (e) {
                    console.error('Error creating arrival rate rule:', e);
// ELIMINADO1090',message:'Error saving rule',data:{error:e.message,rule:rule.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                }
            }
