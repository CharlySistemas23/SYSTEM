...
export const generalLimiter = rateLimit({
    ...
    max: 2000, // Máximo 2000 requests por ventana (aumentado)
    message: {
        success: false,
    ...
    legacyHeaders: false,
    skip: (req) => {
        // Saltar rate limiting para health checks y rutas frecuentes
        const skipPaths = [
            '/health',
            '/api/info',
            '/api/csrf-token',
            '/api/auth/verify', // Se llama frecuentemente para verificar sesión
            '/api/catalogs' // Los catálogos se cargan frecuentemente
        ];
        
        // Excluir rutas de verificación y catálogos
...
export const authLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutos
    max: 20, // Máximo 20 intentos de login por IP (aumentado para permitir reintentos y múltiples métodos)
    message: {
        success: false,
    ...
    skip: (req) => {
        // No aplicar rate limiting a verify (se llama frecuentemente)
        return req.path === '/api/auth/verify' || req.path === '/verify';
    },
    handler: (req, res) => {
        ...
        res.status(429).json({
            success: false,
            error: 'Demasiados intentos de login. Por favor, espera unos minutos antes de intentar nuevamente.',
