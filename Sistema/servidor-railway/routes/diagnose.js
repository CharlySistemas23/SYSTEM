...
// Limpiar sucursales duplicadas
router.post('/fix-duplicates', asyncHandler(async (req, res) => {
    try {
        const { fixDuplicateBranches } = await import('../database/fix-duplicates.js');
        const report = await fixDuplicateBranches();
        
        res.json({
            success: true,
            message: 'Limpieza de duplicados ejecutada exitosamente',
            report: report
        });
    } catch (error) {
        res.status(500).json({
