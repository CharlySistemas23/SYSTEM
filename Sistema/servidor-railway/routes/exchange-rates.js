...
// exchange-rates es global, no necesita ensureOwnBranch (no tiene branch_id)
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    let rate;
    if (existing) {
        // Actualizar existente (updated_at se establece automáticamente en update())
    ...
    // Emitir evento WebSocket (tipos de cambio son globales)
    emitToAll(req.io, existing ? 'exchange-rate-updated' : 'exchange-rate-created', rate);
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    // Emitir evento WebSocket (tipos de cambio son globales)
    if (req.io) {
        emitToAll(req.io, 'exchange-rate-updated', rate);
    }
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    await remove('exchange_rates_daily', id);

    // Emitir evento WebSocket (tipos de cambio son globales)
    emitToAll(req.io, 'exchange-rate-deleted', { id });
