...
router.post('/setup-admin', asyncHandler(async (req, res) => {
    try {
        ...
        // Ejecutar fixAdmin y capturar información de debug
        const debugInfo = { steps: [] };
        await fixAdmin();
        
        // Verificar usuario después de fixAdmin
        const adminUser = await queryOne("SELECT * FROM users WHERE username = 'admin'");
        const pinTest = adminUser && adminUser.pin_hash ? await bcrypt.compare('1234', adminUser.pin_hash) : false;
