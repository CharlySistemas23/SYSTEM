...
router.post('/calculate', asyncHandler(async (req, res) => {
    ...
    if (!passengers || passengers < 1) {
        return res.status(400).json({
            success: false,
            error: 'passengers es requerido y debe ser mayor a 0'
        });
    }
...
router.get('/arrivals', asyncHandler(async (req, res) => {
    ...
    if (dateFrom && !/^\d{4}-\d{2}-\d{2}$/.test(dateFrom)) {
        return res.status(400).json({
            success: false,
            error: 'dateFrom debe tener el formato YYYY-MM-DD'
        });
    }
    ...
    if (dateTo && !/^\d{4}-\d{2}-\d{2}$/.test(dateTo)) {
        return res.status(400).json({
            success: false,
            error: 'dateTo debe tener el formato YYYY-MM-DD'
        });
    }
    ...
    // Siempre retornar array vacío si no hay llegadas (no es un error)
    res.json({
        success: true,
        data: arrivals || [],
        count: arrivals ? arrivals.length : 0
    });
}));
