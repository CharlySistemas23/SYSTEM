...
router.get('/sellers/:id', asyncHandler(async (req, res) => {
    ...
    // Si no existe, retornar objeto vacío en lugar de 404
    if (!seller) {
        return res.json({
            success: true,
            data: null
...
router.put('/sellers/:id', asyncHandler(async (req, res) => {
    ...
    let seller = await queryOne('SELECT * FROM catalog_sellers WHERE id = $1', [id]);
    ...
    if (!seller) {
        if (!name) {
            return res.status(400).json({
                success: false,
                error: 'El nombre es requerido para crear un nuevo vendedor'
            });
        }
        
        seller = await insert('catalog_sellers', {
            id: id,
            name: name,
            barcode: barcode || null,
            branch_id: branch_id || currentBranchId,
            active: active !== undefined ? active : true
        });
        
        emitToBranch(req.io, branch_id || currentBranchId, 'seller-created', seller);
...
router.put('/guides/:id', asyncHandler(async (req, res) => {
    ...
    let guide = await queryOne('SELECT * FROM catalog_guides WHERE id = $1', [id]);
    ...
    if (!guide) {
        if (!name) {
            return res.status(400).json({
                success: false,
                error: 'El nombre es requerido para crear un nuevo guía'
            });
        }
        
        guide = await insert('catalog_guides', {
            id: id,
            name: name,
            barcode: barcode || null,
            agency_id: agency_id || null,
        ...
        emitToBranch(req.io, branch_id || currentBranchId, 'guide-created', guide);
...
router.put('/agencies/:id', asyncHandler(async (req, res) => {
    ...
    let agency = await queryOne('SELECT * FROM catalog_agencies WHERE id = $1', [id]);
    ...
    if (!agency) {
        if (!name) {
            return res.status(400).json({
                success: false,
                error: 'El nombre es requerido para crear una nueva agencia'
            });
        }
        
        agency = await insert('catalog_agencies', {
            id: id,
            name: name || 'Nueva Agencia',
            barcode: barcode || null,
            branch_id: branch_id || currentBranchId,
            active: active !== undefined ? active : true
        });
        
        emitToBranch(req.io, branch_id || currentBranchId, 'agency-created', agency);
        
        return res.status(201).json({
            success: true,
            data: agency,
            message: 'Agencia creada'
        });
    }
