...
router.get('/sessions', asyncHandler(async (req, res) => {
    ...
    if (isAdmin && !requestedBranchId) {
        ...
        queryText = `
            SELECT DISTINCT ON (cs.id)
                   ...
                   b.id as branch_id,
                   (SELECT COUNT(*) FROM cash_movements WHERE session_id = cs.id) as movements_count
    ...
    if (isAdmin && !requestedBranchId) {
        queryText += ` ORDER BY cs.id DESC, b.name ASC, cs.opened_at DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
    } else {
        queryText += ` ORDER BY cs.id DESC, cs.opened_at DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
    }
