...
import { sanitizeString, sanitizeNumber, sanitizeObject } from '../utils/sanitize.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    countQuery = countQuery.replace(/SELECT i\.\*, b\.name as branch_name, b\.id as branch_id/, 'SELECT COUNT(*) as total');
    const countResult = await queryOne(countQuery, params.slice(0, -2)); // Remover limit y offset
    const total = parseInt(countResult?.total || items.length);
    ...
    res.json({
        ...
        limit: finalLimit,
        offset: offset,
        hasMore: offset + itemsWithPhotos.length < total
    });
}));
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const sanitizedBody = sanitizeObject(req.body, {
        ...
        sku: { type: 'string', options: { maxLength: 100 } },
        barcode: { type: 'string', options: { maxLength: 50 } },
        name: { type: 'string', options: { maxLength: 200 } },
        description: { type: 'string', options: { maxLength: 1000 } },
        metal: { type: 'string', options: { maxLength: 50 } },
        stone: { type: 'string', options: { maxLength: 50 } },
        size: { type: 'string', options: { maxLength: 50 } },
        weight: { type: 'number', options: { min: 0 } },
        dimensions: { type: 'string', options: { maxLength: 200 } },
        cost: { type: 'number', options: { min: 0 } },
        price: { type: 'number', options: { min: 0 } },
        stock: { type: 'number', options: { min: 0, integer: true } },
        location: { type: 'string', options: { maxLength: 200 } },
    ...
    // Filtrar solo campos que existen en la tabla inventory_items
    const itemData = {
        id: itemId,
        sku: sku,
    ...
    // Remover campos undefined o null innecesarios para limpiar el objeto
    Object.keys(itemData).forEach(key => {
        if (itemData[key] === undefined || (itemData[key] === null && key !== 'barcode' && key !== 'description' && key !== 'metal' && key !== 'stone' && key !== 'size' && key !== 'weight' && key !== 'dimensions' && key !== 'location' && key !== 'device_id')) {
            delete itemData[key];
        }
    });
