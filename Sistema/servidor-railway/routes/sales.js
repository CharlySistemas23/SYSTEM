...
import { logCreate, logUpdate, logDelete } from '../utils/audit.js';
...
router.post('/', asyncHandler(async (req, res) => {
    // CRÍTICO: Validar que branchId existe
    if (!req.user.branchId) {
        return res.status(400).json({
            success: false,
            error: 'Branch ID no encontrado. Por favor, inicia sesión nuevamente.'
    ...
    // Sanitizar datos del body (sanitizar campos principales, items y payments se sanitizan individualmente)
    const sanitizedBody = sanitizeObject(req.body, {
        folio: { type: 'string', options: { maxLength: 100 } },
        sellerId: { type: 'string', options: {} },
        guideId: { type: 'string', options: {} },
        agencyId: { type: 'string', options: {} },
        customerId: { type: 'string', options: {} },
        passengers: { type: 'number', options: { min: 1, integer: true } },
        currency: { type: 'string', options: { maxLength: 10 } },
        exchangeRate: { type: 'number', options: { min: 0 } },
        subtotal: { type: 'number', options: { min: 0 } },
        discount: { type: 'number', options: { min: 0 } },
        total: { type: 'number', options: { min: 0 } },
        sellerCommission: { type: 'number', options: { min: 0 } },
        guideCommission: { type: 'number', options: { min: 0 } },
        status: { type: 'string', options: { maxLength: 50 } },
        notes: { type: 'string', options: { maxLength: 1000 } }
    });
    
    // Combinar arrays originales con campos sanitizados
    const {
        folio,
        items = [],
        payments = [],
    ...
    for (const item of items) {
        if (!item.name) {
            console.warn('Item sin nombre, saltando:', item);
            continue;
        }
    ...
    try {
        await logCreate(req, 'sale', sale.id, sale, sale.branch_id);
    } catch (auditError) {
        console.warn('Error registrando auditoría de venta:', auditError);
    }

    // Emitir evento WebSocket para actualización en tiempo real
    emitToBranch(req.io, sale.branch_id, 'sale-created', sale);
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    // Registrar en auditoría
    try {
        await logUpdate(req, 'sale', id, existingSale, updatedSale, updatedSale.branch_id || branchId);
    } catch (auditError) {
        console.warn('Error registrando auditoría de actualización de venta:', auditError);
    }

    // Emitir evento WebSocket
    emitToBranch(req.io, updatedSale.branch_id || branchId, 'sale-updated', updatedSale);
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        await logDelete(req, 'sale', id, sale, sale.branch_id);
    } catch (auditError) {
        console.warn('Error registrando auditoría de eliminación de venta:', auditError);
    }
    ...
    // Emitir evento WebSocket
    emitToBranch(req.io, sale.branch_id, 'sale-deleted', { id });
