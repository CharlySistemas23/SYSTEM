...
router.get('/dashboard', asyncHandler(async (req, res) => {
    ...
    const { dateFrom, dateTo, viewAllBranches = 'false', branchId: requestedBranchId } = req.query;
    
    // Determinar qué sucursal(es) consultar
    const isAdmin = req.user.role === 'admin' || req.user.permissions?.includes('all');
    
    // Si admin solicita una sucursal específica, usarla. Si no, ver todas si viewAllBranches=true
    const filterBranchId = isAdmin && requestedBranchId ? requestedBranchId : null;
    const shouldViewAll = isAdmin && viewAllBranches === 'true' && !requestedBranchId;
...
router.get('/sales/detailed', asyncHandler(async (req, res) => {
    ...
    queryText += ` ORDER BY s.id DESC, s.created_at DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
...
router.get('/inventory', asyncHandler(async (req, res) => {
    ...
    let queryText = `
        SELECT 
            i.*,
            (SELECT COUNT(*) FROM inventory_photos WHERE inventory_item_id = i.id) as photos_count,
            (SELECT COUNT(*) FROM inventory_certificates WHERE inventory_item_id = i.id) as certificates_count
        FROM inventory_items i
        WHERE i.branch_id = $1
    `;
    const params = [branchId];
