...
import { sanitizeString, sanitizeEmail, sanitizeObject } from '../utils/sanitize.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    countQuery = countQuery.replace(/SELECT \*/, 'SELECT COUNT(*) as total');
    const countResult = await queryOne(countQuery, params.slice(0, -2));
    const total = parseInt(countResult?.total || customers.length);
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const sanitizedBody = sanitizeObject(req.body, {
        ...
        phone: { type: 'string', options: { maxLength: 50 } },
        address: { type: 'string', options: { maxLength: 500 } },
    ...
    emitToBranch(branchId, 'customer-created', customer);
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    emitToBranch(req.io, targetBranchId, 'customer-updated', updatedCustomer);
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    // Emitir evento WebSocket
    emitToBranch(req.io, customer.branch_id || branchId, 'customer-deleted', { id });
