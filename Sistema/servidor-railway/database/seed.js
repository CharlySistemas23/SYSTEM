...
export async function seedDatabase() {
    try {
            ...
            if (existingCommissionRules.length === 0) {
                ...
                for (const rule of commissionRules) {
                    try {
                        ...
                        console.log('✅ Regla de comisión creada:', rule.id);
                    } catch (error) {
        ...
        // 1. Verificar que las 4 sucursales existan siempre (no solo si no hay ninguna)
        console.log('📦 Verificando que todas las sucursales existan...');
        ...
        const allBranches = await query('SELECT * FROM catalog_branches');

        for (const requiredBranch of requiredBranches) {
            // Verificar por ID
            const existsById = allBranches.find(b => b.id === requiredBranch.id);
            // Verificar por nombre (case-insensitive) para evitar duplicados
            const existsByName = allBranches.find(b => 
            ...
            if (!existsById && !existsByName) {
                ...
                if (!exists.active) {
                    const { update } = await import('../config/database.js');
                    await update('catalog_branches', exists.id, { active: true });
                    console.log(`✅ Sucursal ${exists.name} reactivada`);
                }
                if (!existingBranch) {
        ...
        if (!existingBranch) {
            existingBranch = await queryOne('SELECT * FROM catalog_branches WHERE active = true LIMIT 1');
        }

        if (existingBranch) {
            console.log('✅ Sucursal por defecto:', existingBranch.name);
        } else {
            console.log('⚠️ No se pudo obtener sucursal por defecto');
        }
        ...
        // Verificar que todos los usuarios tengan branch_id
        console.log('');
        console.log('🔍 Verificando integridad de datos...');
        const usersWithoutBranch = await query('SELECT * FROM users WHERE (branch_id IS NULL OR branch_id = \'\') AND active = true');
        if (usersWithoutBranch.length > 0 && branchId) {
            const { update } = await import('../config/database.js');
            for (const user of usersWithoutBranch) {
                await update('users', user.id, { branch_id: branchId });
                console.log(`✅ Branch_id asignado a usuario: ${user.username}`);
            }
        }

        // Verificar que todos los empleados tengan branch_id
        const employeesWithoutBranch = await query('SELECT * FROM employees WHERE (branch_id IS NULL OR branch_id = \'\') AND active = true');
        if (employeesWithoutBranch.length > 0 && branchId) {
            const { update } = await import('../config/database.js');
            for (const employee of employeesWithoutBranch) {
                await update('employees', employee.id, { branch_id: branchId });
                console.log(`✅ Branch_id asignado a empleado: ${employee.name}`);
            }
        }
