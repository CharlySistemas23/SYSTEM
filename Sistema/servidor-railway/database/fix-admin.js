...
async function fixAdmin() {
    try {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fix-admin.js:119',message:'Generando hashes de credenciales',data:{hasAdminUser:!!adminUser},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B'})}).catch(()=>{});
        // #endregion
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fix-admin.js:123',message:'Hashes generados, verificando PIN',data:{pinHashLength:pinHash.length,passwordHashLength:passwordHash.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        // #endregion
        ...
        if (!adminUser) {
            ...
            try {
                ...
                // #region agent log
                const pinTest = await bcrypt.compare('1234', adminUser.pin_hash);
                fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fix-admin.js:139',message:'Usuario admin creado',data:{userId:adminUser.id,username:adminUser.username,active:adminUser.active,hasPinHash:!!adminUser.pin_hash,pinTestValid:pinTest},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B'})}).catch(()=>{});
                // #endregion
            } catch (error) {
                if (error.code === '23505') {
            ...
            if (adminUser.pin_hash) {
                ...
                // #region agent log
                fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fix-admin.js:195',message:'Verificando PIN hash existente',data:{hasPinHash:!!adminUser.pin_hash,pinValid:pinValid,userId:adminUser.id,active:adminUser.active},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B,C'})}).catch(()=>{});
                // #endregion
        ...
        // #region agent log
        const finalPinTest = adminUser ? await bcrypt.compare('1234', adminUser.pin_hash) : false;
        fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fix-admin.js:217',message:'Verificación final usuario admin',data:{userExists:!!adminUser,userId:adminUser?.id,username:adminUser?.username,active:adminUser?.active,hasPinHash:!!adminUser?.pin_hash,finalPinTest:finalPinTest},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B,C'})}).catch(()=>{});
        // #endregion
