...
async function fixAdmin() {
    try {
        ...
        // 8. Verificar reglas de comisión (no crear automáticamente - el usuario ya las pasó)
        console.log('8️⃣ Verificando reglas de comisión...');
        const existingCommissionRules = await query('SELECT * FROM commission_rules LIMIT 1');
        if (existingCommissionRules.length === 0) {
            console.log('⚠️ No hay reglas de comisión. El usuario debe crearlas manualmente.');
        } else {
            console.log(`✅ Reglas de comisión encontradas: ${existingCommissionRules.length}`);
        }
        
        // 9. Verificar que todos los usuarios tengan branch_id
        console.log('');
        console.log('9️⃣ Verificando branch_id en usuarios...');
        const usersWithoutBranch = await query('SELECT * FROM users WHERE branch_id IS NULL AND active = true');
        if (usersWithoutBranch.length > 0) {
            console.log(`⚠️ ${usersWithoutBranch.length} usuarios sin branch_id. Asignando sucursal por defecto...`);
            ...
            if (defaultBranch) {
                ...
                console.error('❌ No hay sucursales para asignar a usuarios');
            }
        } else {
            console.log('✅ Todos los usuarios tienen branch_id');
        }
