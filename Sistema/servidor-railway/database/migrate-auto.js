...
const TABLE_ORDER = [
    ...
    // Fase 5: Tablas de sistema y auditoría
    'audit_logs',            // Depende de users y catalog_branches
];
...
export async function migrate() {
    try {
        ...
        try {
            const { setSchemaVersion, SCHEMA_VERSION } = await import('./schema-version.js');
            await setSchemaVersion(SCHEMA_VERSION);
        } catch (versionError) {
            console.warn('⚠️ No se pudo actualizar la versión del esquema:', versionError.message);
        }
        
        // Crear tabla de auditoría si no existe (desde archivo separado)
        try {
            ...
            if (fs.existsSync(auditSchemaPath)) {
                const auditSQL = fs.readFileSync(auditSchemaPath, 'utf8');
                const auditStatements = extractSQLStatements(auditSQL);
                ...
                for (const statement of auditStatements) {
                    try {
                        ...
                        if (!error.message.includes('already exists')) {
                            console.warn('⚠️ Error creando tabla de auditoría:', error.message);
                        }
                    }
                }
            ...
            console.warn('⚠️ No se pudo crear tabla de auditoría:', auditError.message);
        }
