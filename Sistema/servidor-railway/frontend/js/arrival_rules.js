...
const ArrivalRules = {
    ...
    async calculateArrivalFee(agencyId, branchId, passengers, unitType = null, dateYYYYMMDD = null) {
        try {
            // ELIMINADOarrival_rules.js:13',message:'calculateArrivalFee START',data:{agencyId,branchId,passengers,unitType,dateYYYYMMDD},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            const date = dateYYYYMMDD || Utils.formatDate(new Date(), 'YYYY-MM-DD');
            const agency = await DB.get('catalog_agencies', agencyId);
            if (!agency) {
        // ELIMINADOarrival_rules.js:18',message:'Agency not found',data:{agencyId},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            ...
            // ELIMINADOarrival_rules.js:24',message:'All rules loaded',data:{totalRules:allRules.length,agencyRules:allRules.filter(r=>r.agency_id===agencyId).map(r=>({id:r.id,min:r.min_passengers,max:r.max_passengers,branch:r.branch_id,unit:r.unit_type}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            ...
            // ELIMINADOarrival_rules.js:40',message:'Active rules after filter',data:{count:activeRules.length,rules:activeRules.map(r=>({id:r.id,min:r.min_passengers,max:r.max_passengers,branch:r.branch_id,unit:r.unit_type,fee_type:r.fee_type,flat_fee:r.flat_fee}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            ...
            // ELIMINADOarrival_rules.js:68',message:'Rule found in range',data:{found:!!applicableRule,rule:applicableRule?{id:applicableRule.id,min:applicableRule.min_passengers,max:applicableRule.max_passengers}:null,passengers},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        ...
        // ELIMINADOarrival_rules.js:87',message:'Rule found for extra calculation',data:{found:!!applicableRule,rule:applicableRule?{id:applicableRule.id,min:applicableRule.min_passengers,max:applicableRule.max_passengers}:null,passengers,rulesWithMaxCount:rulesWithMax.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            }

            if (!applicableRule) {
        // ELIMINADOarrival_rules.js:75',message:'No applicable rule found',data:{agencyName,passengers,unitType,branchId,activeRulesCount:activeRules.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                return { 
                    calculatedFee: 0, 
        ...
        // ELIMINADOarrival_rules.js:86',message:'Flat fee calculation',data:{flat_fee:applicableRule.flat_fee,calculatedFee,passengers,maxPax:applicableRule.max_passengers,extra_per_passenger:applicableRule.extra_per_passenger},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
        ...
        // ELIMINADOarrival_rules.js:94',message:'Extra passengers calculated',data:{extraPax,extraPerPax:applicableRule.extra_per_passenger,calculatedFee},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                    }
                }
            } else {
                // Tarifa por pasajero
        ...
        // ELIMINADOarrival_rules.js:100',message:'Per passenger calculation',data:{ratePerPax,passengers,calculatedFee},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
            ...
            // ELIMINADOarrival_rules.js:112',message:'Final calculated fee',data:{calculatedFee,agencyName,passengers,rule:applicableRule.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
