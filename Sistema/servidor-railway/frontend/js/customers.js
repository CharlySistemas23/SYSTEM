...
const Customers = {
    ...
    setupWebSocketListeners() {
        ...
        SocketManager.on('customer-created', (data) => {
            if (this.shouldShowCustomer(data)) {
                this.loadCustomers();
                Utils.showNotification(`Nuevo cliente: ${data.name}`, 'success');
            }
        });
        
        SocketManager.on('customer-updated', (data) => {
            if (this.shouldShowCustomer(data)) {
                this.loadCustomers();
            }
        });
        
        SocketManager.on('customer-deleted', (data) => {
            // Remover cliente de la lista si está visible
            const customerElement = document.querySelector(`[data-customer-id="${data.id}"]`);
            if (customerElement) {
    ...
    // Verificar si un cliente debe mostrarse según el filtro de sucursal activo
    shouldShowCustomer(customer) {
        if (typeof BranchSelector !== 'undefined' && BranchSelector.isAdmin && BranchSelector.selectedBranchId) {
            return customer.branch_id === BranchSelector.selectedBranchId;
        }
        return true;
    },
    ...
    async loadCustomers() {
        try {
            ...
            // Los clientes no tienen branch_id, pero las ventas sí
            ...
            // Si no es admin, filtrar clientes que solo tienen ventas en la sucursal actual
                ...
                <td class="col-name">
                    <div class="customer-name-cell">
                        ...
                        <div class="customer-name-info">
                            ...
                            ${customer.branch_name ? `<div style="margin-top: 2px;"><span class="badge badge-info" style="font-size: 9px; padding: 1px 4px;">🏢 ${customer.branch_name}</span></div>` : ''}
                        </div>
                    </div>
                </td>
            ...
            <div class="customer-card" data-customer-id="${customer.id}">
                <div class="customer-card-header">
                    ...
                    <div class="customer-card-info">
                        ...
                        <span class="segment-badge ${segmentClass}">${customer.autoSegment || customer.segment || 'Sin segmento'}</span>
                        ${customer.branch_name ? `<span class="badge badge-info" style="font-size: 10px; padding: 2px 6px; margin-top: 4px; display: inline-block;">🏢 ${customer.branch_name}</span>` : ''}
                    </div>
    ...
    async saveCustomer(customerId) {
        ...
        // Validar nombre
        if (typeof Validators !== 'undefined') {
            if (!Validators.string(nameInput?.value, { required: true, minLength: 2, maxLength: 100 }).valid) {
                isValid = false;
                if (nameInput) {
                    nameInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El nombre es requerido y debe tener entre 2 y 100 caracteres';
            ...
            if (emailInput?.value) {
                const emailResult = Validators.email(emailInput.value);
                if (!emailResult.valid) {
                    isValid = false;
                    emailInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = emailResult.error;
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
            ...
            if (phoneInput?.value) {
                ...
                if (!phoneResult.valid) {
                    ...
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = phoneResult.error;
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
        ...
        const existingCustomer = isEdit ? await DB.get('customers', customerId) : null;
        
        if (isEdit && !existingCustomer) {
            Utils.showNotification('El cliente que intentas editar no existe', 'error');
            return;
        }
        
        const customer = {
            // Solo incluir ID si es edición
            ...(isEdit ? { id: customerId } : {}),
        ...
        try {
            let savedCustomer;
            if (isEdit) {
                savedCustomer = await DB.put('customers', customer);
            } else {
                savedCustomer = await DB.add('customers', customer);
            }
            ...
            Utils.showNotification(isEdit ? 'Cliente actualizado' : 'Cliente creado', 'success');
            UI.closeModal();
            this.loadCustomers();
        } catch (error) {
            console.error('Error guardando cliente:', error);
            const errorMessage = error.message || error.error || 'Error desconocido';
            Utils.showNotification('Error al guardar cliente: ' + errorMessage, 'error');
        }
    ...
    async exportCustomers() {
        try {
            ...
            const exportData = customers.map(c => {
                const customerSales = sales.filter(s => s.customer_id === c.id && s.status === 'completada');
                const totalSpent = customerSales.reduce((sum, s) => sum + (s.total || 0), 0);
            ...
            UI.showModal('Exportar Clientes', body, '<button class="btn-secondary" onclick="UI.closeModal()">Cancelar</button>');
