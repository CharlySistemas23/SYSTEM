...
const UserManager = {
    ...
    async handleBarcodeInput(barcode) {
        try {
            // Buscar empleado por barcode usando el backend
            if (typeof API !== 'undefined' && API.getEmployees) {
                try {
                    const employees = await API.getEmployees({ barcode: barcode });
                    const employee = Array.isArray(employees) ? employees.find(e => e.barcode === barcode && e.active) : null;
                    ...
                    console.warn('Error buscando empleado por barcode:', e);
                }
            }
            
            // Intentar buscar por username usando el backend
            if (typeof API !== 'undefined' && API.getUsers) {
                try {
                    const users = await API.getUsers();
                    const user = Array.isArray(users) ? users.find(u => u && u.username && u.username.toLowerCase() === barcode.toLowerCase() && u.active) : null;
                    ...
                    console.warn('Error buscando usuario por username:', e);
                }
            }
        } catch (e) {
