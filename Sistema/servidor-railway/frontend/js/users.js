...
const UserManager = {
    ...
    async init() {
        ...
        // Verificar token guardado rápidamente (sin petición HTTP si no hay token)
        const token = localStorage.getItem('api_token');
        const savedUser = localStorage.getItem('api_user');
        
        if (token && savedUser) {
            // Hay token guardado, verificar rápidamente
            try {
                ...
                if (isValid) {
                    ...
                    this.hideLogin();
                    return; // Ya hay sesión válida, no hacer autoLogin
                }
            } catch (e) {
                // Token inválido, continuar con autoLogin
                console.log('Token inválido, intentando autoLogin...');
            }
        }
        
        // Si no hay sesión válida, intentar login automático (más rápido que checkAuth completo)
    ...
    async autoLogin() {
        try {
            ...
            // Intentar login directamente primero (más rápido)
            // Si falla, entonces intentar setup-admin
            try {
                ...
                if (response.success && response.token) {
                    ...
                    // Ocultar login inmediatamente (antes de otras inicializaciones)
                    this.hideLogin();
                    
                    // Inicializar componentes en paralelo (más rápido)
                    const initPromises = [];
                    
                    // BranchManager (crítico - debe estar antes de onLoginSuccess)
                    if (typeof BranchManager !== 'undefined') {
                        initPromises.push(
                            BranchManager.init().then(async () => {
                            ...
                            if (branchId) {
                                ...
                                console.error('Error inicializando BranchManager:', e);
                            })
                        );
                    }
                    
                    // Esperar BranchManager (crítico)
                    await Promise.all(initPromises);
                    
                    // CRÍTICO: Cargar datos del sistema antes de mostrar UI
                    if (typeof App !== 'undefined' && App.loadSystemData) {
                        try {
                            console.log('🔄 Cargando datos críticos del sistema...');
                            // Cargar solo datos críticos (catálogos) antes de mostrar UI
                            await App.loadSystemData(true); // criticalOnly = true
                            console.log('✅ Datos críticos cargados');
                            
                            // Cargar datos no críticos en background después
                            App.loadSystemData(false).catch(e => {
                                console.error('Error cargando datos no críticos:', e);
                            });
                        } catch (e) {
                            console.error('⚠️ Error cargando datos críticos:', e);
                            // Continuar de todas formas, pero mostrar advertencia
                            if (typeof Utils !== 'undefined' && Utils.showNotification) {
                                Utils.showNotification('Algunos datos no se cargaron. El sistema puede tener funcionalidad limitada.', 'warning');
                            }
                        }
                    }
                    
                    // WebSocket y otros componentes en background (no bloquean)
                    if (typeof SocketManager !== 'undefined') {
                        SocketManager.init().catch(e => {
                            console.error('Error inicializando SocketManager:', e);
                        });
                    }
                    
                    // Mostrar sistema (ahora con datos críticos cargados)
                ...
                // Si falla, intentar setup-admin solo si el error indica que el usuario no existe
                if (loginError.message && (loginError.message.includes('no encontrado') || loginError.message.includes('incorrectos'))) {
                    try {
                        ...
                        if (setupResponse.ok) {
                            ...
                            if (setupData.success) {
                                console.log('✅ Usuario admin configurado. Reintentando login...');
                                // Reintentar login inmediatamente (sin delay)
                                const retryResponse = await API.login('admin', '1234');
                                if (retryResponse.success && retryResponse.token) {
                                    this.currentUser = retryResponse.user;
                                    this.currentEmployee = retryResponse.employee;
                                    localStorage.setItem('current_user_id', retryResponse.user.id);
                                    if (retryResponse.employee) {
                                        localStorage.setItem('current_employee_id', retryResponse.employee.id);
                                    }
                                    if (typeof SocketManager !== 'undefined') {
                                    ...
                                    if (typeof BranchManager !== 'undefined') {
                                        ...
                                        const branchId = retryResponse.user.branchId || retryResponse.employee?.branchId;
                                        if (branchId) {
                                            await BranchManager.setCurrentBranch(branchId);
                                        }
                                    }
                        ...
                        console.log('⚠️ Setup admin falló:', setupError.message);
                    }
                }
                
                // Si todo falla, mostrar pantalla de login
                this.showLogin();
        ...
        try {
            ...
            try {
                ...
                if (response.success && response.token) {
                    ...
                    // Asegurar que API.currentUser también se actualice
                    if (typeof API !== 'undefined') {
                        API.currentUser = response.user;
                        // Actualizar localStorage también desde API
                        localStorage.setItem('api_user', JSON.stringify(response.user));
                    }
