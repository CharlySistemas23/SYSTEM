...
const DB = {
    ...
    storeToEndpoint: {
        ...
        'daily_profit_reports': '/api/profit/reports'
    },
    ...
    async getAll(storeName, indexName = null, query = null, options = {}) {
        ...
        try {
            ...
            // Si el error es 404 y es un store que puede estar vacío, solo retornar array vacío sin loguear
            if (error.message && error.message.includes('404')) {
                // Para algunos stores, un 404 puede significar simplemente que no hay datos
                const silentStores = ['tourist_report_lines', 'sale_items', 'sale_payments'];
                if (silentStores.includes(storeName)) {
                    return []; // Retornar array vacío sin loguear
                }
            }
            
            // Si el error es que la tabla no existe, intentar auto-reparación
            if (error.message && (error.message.includes('no existe') || error.message.includes('does not exist') || error.message.includes('Tabla no existe') || error.message.includes('relation') && error.message.includes('does not exist'))) {
                console.warn(`⚠️ Tabla para ${storeName} no existe. Intentando auto-reparación...`);
                
                // Intentar auto-reparación automáticamente (solo una vez por sesión)
                const repairKey = `autoRepairAttempted_${storeName}`;
                    ...
                    try {
                        ...
                        if (token) {
                            ...
                            if (repairResponse.ok) {
                                console.log(`✅ Auto-reparación ejecutada para ${storeName}`);
                                // Retornar array vacío, el sistema se reparará en background
                            }
                        }
                    } catch (repairError) {
