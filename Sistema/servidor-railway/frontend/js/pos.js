...
Object.assign(POS, {
    ...
    async init() {
        ...
        // Verificar que los catálogos estén cargados antes de continuar
        // Si no están cargados, esperar a que App.loadSystemData los cargue
        ...
        while (!catalogsLoaded && attempts < maxAttempts) {
            try {
                ...
                if (sellers.length > 0 || guides.length > 0 || agencies.length > 0) {
                    catalogsLoaded = true;
                    break;
                }
                ...
                if (attempts < maxAttempts - 1) {
                    ...
                    // Último intento: cargar catálogos directamente
                    console.log('⚠️ Catálogos no cargados, cargando directamente...');
    ...
    setupWebSocketListeners() {
        ...
        SocketManager.on('inventory-item-created', (data) => {
            if (this.shouldShowItem(data)) {
                ...
                Utils.showNotification(`Nuevo producto disponible: ${data.name}`, 'success');
            }
        });
        ...
        SocketManager.on('inventory-item-deleted', (data) => {
            // Remover producto de la vista si está visible
            const productElement = document.querySelector(`[data-item-id="${data.id}"]`);
            if (productElement) {
        ...
        SocketManager.on('sale-created', (data) => {
            // Si es de la sucursal actual, actualizar contador de ventas del día
            if (this.shouldShowSale(data)) {
                this.updateTodaySalesCount();
            }
        });
    },
    
    // Verificar si un item debe mostrarse según el filtro de sucursal activo
    shouldShowItem(item) {
        if (typeof BranchSelector !== 'undefined' && BranchSelector.isAdmin && BranchSelector.selectedBranchId) {
            return item.branch_id === BranchSelector.selectedBranchId;
        }
        return true;
    },
    
    // Verificar si una venta debe mostrarse según el filtro de sucursal activo
    shouldShowSale(sale) {
        if (typeof BranchSelector !== 'undefined' && BranchSelector.isAdmin && BranchSelector.selectedBranchId) {
            return sale.branch_id === BranchSelector.selectedBranchId;
        }
        return true;
    },
    ...
    async diagnoseSystem() {
        ...
        try {
            // 1. Verificar base de datos (ahora es wrapper de API)
            if (!DB || !DB.initialized) {
                console.error('❌ POS: Base de datos no disponible');
                return;
            }
            console.log('✅ POS: Base de datos disponible (usando Backend API)');
    ...
    setupEventListeners() {
        ...
        if (searchInput) {
            ...
            searchInput.addEventListener('input', Utils.debounce(async (e) => {
                ...
                // Remover lista de autocompletado anterior
                if (autocompleteList) {
                    ...
                    if (suggestions.length > 0) {
                        ...
                        suggestions.forEach((suggestion, idx) => {
                            ...
                            item.innerHTML = `
                                ...
                                <div style="font-size: 11px; color: var(--color-text-secondary);">SKU: ${suggestion.sku}</div>
                            `;
    ...
    async loadProducts() {
        ...
        try {
            ...
            // Obtener items - primero intentar sin filtro de sucursal para debug
            let allItems = await DB.getAll('inventory_items') || [];
            console.log(`POS: Total items en BD: ${allItems.length}`);
            ...
            // Filtrar por sucursal si está configurado - MÁS FLEXIBLE
            let items = allItems;
            if (currentBranchId) {
                items = allItems.filter(item => {
                    // Incluir items de la sucursal actual, sin branch_id, o con branch_id null/undefined
                    return !item.branch_id || item.branch_id === currentBranchId || item.branch_id === null || item.branch_id === undefined;
                });
                console.log(`POS: Items después de filtrar por sucursal: ${items.length}`);
            }
            
            // DEBUG: Mostrar información de todos los items
            console.log('POS: Análisis de items:');
            ...
            if (items.length === 0) {
                ...
                const disponibleCount = allItems.filter(i => i.status === 'disponible').length;
                const conStockCount = allItems.filter(i => (i.stock_actual ?? 1) > 0).length;
                const disponibleConStock = allItems.filter(i => i.status === 'disponible' && (i.stock_actual ?? 1) > 0).length;
                ...
                container.innerHTML = `
                    <div class="pos-loading-products" style="text-align: center; padding: 40px;">
                        ...
                        <div style="font-size: 12px; color: var(--color-text-secondary); margin-bottom: 20px; text-align: left; max-width: 500px; margin-left: auto; margin-right: auto;">
                            ...
                            • Total productos en BD: ${allItems.length}<br>
    ...
    async selectProduct(itemId) {
        ...
        try {
            ...
            const existingInCart = this.cart.find(c => c.id === item.id);
            const totalInCart = existingInCart ? existingInCart.quantity : 0;
            
            if (currentStock < (totalInCart + requestedQuantity)) {
                Utils.showNotification(
                    `Stock insuficiente. Disponible: ${currentStock}, Solicitado: ${totalInCart + requestedQuantity}`,
                    'error'
                );
                return;
            }
    ...
    async completeSale() {
        ...
        // Variables para acumular costos y comisiones
        let totalCOGS = 0;
        let totalSellerCommission = 0;
        let totalGuideCommission = 0;

        // Preparar items de venta con toda la información necesaria
        const saleItems = [];
        ...
        for (const item of this.cart) {
            ...
            const itemCost = currentItem?.cost || 0;
            const itemCOGS = itemCost * item.quantity;
            ...
            const commissionAmount = sellerCommission + guideCommission;
            
            // Preparar item para el backend (formato esperado por el backend)
            saleItems.push({
                id: Utils.generateId(),
                inventoryItemId: item.id,
                name: currentItem.name || item.name || item.sku,
            ...
            // Preparar actualización de inventario (se hará después de crear la venta)
        ...
        // Preparar pagos
        const payments = await this.preparePayments();
        
        // Crear venta usando API directamente (el backend creará items y pagos automáticamente)
        const saleData = {
            id: Utils.generateId(),
            folio: folio,
            items: saleItems,
            ...
            guideCommission: totalGuideCommission,
            status: 'completada',
            notes: ''
        };
        
        // Crear venta en el backend (esto creará automáticamente items y pagos)
        let sale;
        try {
            if (typeof API !== 'undefined' && API.createSale) {
                sale = await API.createSale(saleData);
            } else {
                throw new Error('API.createSale no está disponible');
            }
        } catch (error) {
            console.error('Error creando venta:', error);
            Utils.showNotification('Error al crear la venta: ' + error.message, 'error');
            throw error;
        }
        ...
        for (const update of inventoryUpdates) {
            ...
            await DB.add('inventory_logs', {
                ...
                stock_before: update.item.stock_actual ?? 1,
                stock_after: update.newStock,
                reason: 'venta',
                notes: `Venta ${folio} - Cantidad: ${update.quantity}`,
        ...
        if (typeof Costs !== 'undefined' && totalCOGS > 0) {
            await Costs.registerCOGS(sale.id, totalCOGS, branchId, folio);
        }

        // Registrar comisiones automáticamente
        if (typeof Costs !== 'undefined') {
            if (totalSellerCommission > 0 && sellerId) {
                await Costs.registerCommission(sale.id, totalSellerCommission, branchId, folio, 'seller', sellerId);
            }
            if (totalGuideCommission > 0 && guideId) {
                await Costs.registerCommission(sale.id, totalGuideCommission, branchId, folio, 'guide', guideId);
            }
        }

        // Registrar comisiones bancarias de pagos
        for (const payment of payments) {
            if (payment.bankCommission > 0 && typeof Costs !== 'undefined') {
                await Costs.registerBankCommission(
                    sale.id, 
    ...
    // Preparar pagos en formato para el backend (sin guardar aún)
    ...
    // Guardar pagos (para compatibilidad con código existente, pero ya no se usa en completeSale)
    async savePayments(saleId) {
        // Esta función ya no se usa - los pagos se envían junto con la venta
        // Se mantiene por compatibilidad pero no hace nada
        console.warn('savePayments() está obsoleto - los pagos se envían junto con la venta');
    },
