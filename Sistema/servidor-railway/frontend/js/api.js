    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.debug && isLoginEndpoint) {
                    // #region agent log
                    fetch('http://127.0.0.1:7242/ingest/d085ffd8-d37f-46dc-af23-0f9fbbe46595',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'api.js:83',message:'Debug info de login error',data:errorData.debug,timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A,B,C,E'})}).catch(()=>{});
                    // #endregion
                }
                ...
                if (errorData.debug) loginError.debug = errorData.debug;
                throw loginError;
            }
