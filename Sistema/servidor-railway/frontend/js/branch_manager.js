...
const BranchManager = {
     ...
     * Solo se debe llamar DESPUÉS de que el usuario haya iniciado sesión
     */
    async init() {
        try {
            ...
            if (typeof API !== 'undefined' && !API.token && !hasUser) {
                // Si no hay token ni usuario simulado, solo usar la sucursal guardada en localStorage
                ...
                if (savedBranchId) {
                    ...
                    if (branchEl) {
                        branchEl.textContent = savedBranchId;
                    }
                }
                console.log('BranchManager initialized (sin token, usando localStorage):', this.currentBranchId);
                return;
            }
                ...
                if (employee.role === 'admin' || employee.role === 'manager') {
                    ...
                    if (branchIds.length > 0) {
                        ...
                        try {
                            ...
                            // Si falla, usar la guardada en localStorage
                            const savedBranchId = localStorage.getItem('current_branch_id');
                            if (savedBranchId) {
                                this.currentBranchId = savedBranchId;
                                localStorage.setItem('current_branch_id', savedBranchId);
                            }
                        }
                    }
                } else {
            ...
            // Si hay error, al menos guardar la sucursal en localStorage si existe
            if (e.requiresAuth || (e.message && e.message.includes('Token no proporcionado'))) {
                const savedBranchId = localStorage.getItem('current_branch_id');
                if (savedBranchId) {
                    this.currentBranchId = savedBranchId;
                }
    ...
    async setCurrentBranch(branchId) {
        try {
            // Verificar si hay token antes de hacer peticiones a la API
            // Si hay usuario simulado, continuar sin token
            const hasUser = typeof UserManager !== 'undefined' && UserManager.currentUser;
            
            if (typeof API !== 'undefined' && !API.token && !hasUser) {
                // Si no hay token ni usuario simulado, usar la sucursal guardada en localStorage sin hacer petición
                this.currentBranchId = branchId;
                localStorage.setItem('current_branch_id', branchId);
                // Intentar actualizar UI sin datos completos
                const branchEl = document.getElementById('current-branch');
                if (branchEl) {
                    branchEl.textContent = branchId;
                }
                return true;
            }
            ...
            if (!branch) {
                ...
                try {
                    ...
                    const firstBranch = allBranches.find(b => b.active) || allBranches[0];
                    if (firstBranch) {
                        console.log(`⚠️ Usando sucursal alternativa: ${firstBranch.name}`);
                        branchId = firstBranch.id;
                        return await this.setCurrentBranch(firstBranch.id);
                    }
                } catch (e) {
                    console.error('Error buscando sucursal alternativa:', e);
                }
            ...
            // Usar tanto el evento estándar como el personalizado para compatibilidad
            const branchChangedEvent = new CustomEvent('branch-changed', { 
                detail: { branchId, branch, previousBranchId: this.currentBranchId } 
            });
            window.dispatchEvent(branchChangedEvent);
            
            // También emitir evento legacy para compatibilidad
            if (typeof document !== 'undefined') {
                document.dispatchEvent(branchChangedEvent);
            }
            
            // Notificar a todos los módulos que escuchan cambios de sucursal
            if (typeof Utils !== 'undefined' && Utils.EventBus) {
                Utils.EventBus.emit('branch-changed', { branchId, branch });
            }
            
            // Emitir evento adicional para módulos que usan addEventListener
            ...
            // Forzar recarga de datos en módulos principales
            if (typeof App !== 'undefined' && App.loadSystemData) {
                // Recargar datos del sistema después de un pequeño delay
                setTimeout(async () => {
                    try {
                        await App.loadSystemData();
                    } catch (e) {
                        console.warn('Error recargando datos del sistema:', e);
                    }
                }, 100);
            }
            
            console.log(`✅ Sucursal cambiada a: ${branch.name} (${branchId})`);
            return true;
        } catch (e) {
            // Si el error es por falta de autenticación, solo guardar en localStorage
            if (e.requiresAuth || (e.message && e.message.includes('Token no proporcionado'))) {
                this.currentBranchId = branchId;
                localStorage.setItem('current_branch_id', branchId);
                return true;
