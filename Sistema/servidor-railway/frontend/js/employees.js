...
const Employees = {
            ...
            while (!catalogsLoaded && attempts < maxAttempts) {
                try {
                    ...
                    if (attempts < maxAttempts - 1) {
                        ...
                        console.log('⚠️ Catálogos no cargados en Employees, continuando...');
                        catalogsLoaded = true;
                    }
                } catch (e) {
                    console.error('Error verificando catálogos en Employees:', e);
                    catalogsLoaded = true;
                    break;
                }
            }
    ...
    async loadEmployees() {
        ...
        setTimeout(() => {
            ...
            // Agregar debounce a la búsqueda de empleados
            const searchInput = document.getElementById('employee-search-input');
            if (searchInput && typeof Utils !== 'undefined' && Utils.debounce) {
                searchInput.addEventListener('input', Utils.debounce((e) => {
                    const statusFilter = document.querySelector('.btn-secondary.btn-sm.active[data-filter]')?.dataset.filter || 'all';
                    const roleFilter = document.getElementById('employee-role-filter')?.value || '';
                    const branchFilter = document.getElementById('employee-branch-filter')?.value || '';
                    this.filterEmployees(e.target.value, statusFilter, roleFilter, branchFilter);
                }, 300));
            }
        }, 100);
    },
    ...
    async loadUsers() {
        ...
        // Agregar debounce a la búsqueda de usuarios
        setTimeout(() => {
            ...
            if (searchInput && typeof Utils !== 'undefined' && Utils.debounce) {
                searchInput.addEventListener('input', Utils.debounce((e) => {
                    ...
                    this.filterUsers(e.target.value, statusFilter, roleFilter);
                }, 300));
            }
        }, 100);
    },
            ...
            if (typeof Validators !== 'undefined') {
                const validations = {
                    ...
                    barcode: {
                        ...
                        options: { required: false, minLength: 8, maxLength: 50 }
                    }
                };

                const result = Validators.validateFields(validations);
                ...
                // Fallback a validaciones básicas
