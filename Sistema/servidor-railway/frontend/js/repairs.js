...
const Repairs = {
    ...
    setupWebSocketListeners() {
        ...
        SocketManager.on('repair-created', (data) => {
            if (this.shouldShowRepair(data)) {
                this.loadRepairs();
                Utils.showNotification(`Nueva reparación: ${data.folio}`, 'success');
            }
        });
        
        SocketManager.on('repair-updated', (data) => {
            if (this.shouldShowRepair(data)) {
                this.loadRepairs();
            }
        });
    ...
    // Verificar si una reparación debe mostrarse según el filtro de sucursal activo
    shouldShowRepair(repair) {
        if (typeof BranchSelector !== 'undefined' && BranchSelector.isAdmin && BranchSelector.selectedBranchId) {
            return repair.branch_id === BranchSelector.selectedBranchId;
        }
        return true;
    },
    ...
    async loadRepairs() {
        try {
            ...
            // Las reparaciones pueden tener branch_id, filtrar usando filtrado automático
            let repairs = await DB.getAll('repairs', null, null, { 
    ...
    async saveRepair(repairId) {
        ...
        const customerNameInput = document.getElementById('repair-customer-name') || document.getElementById('repair-customer');
        const descriptionInput = document.getElementById('repair-description');
        const statusInput = document.getElementById('repair-status');
        const costInput = document.getElementById('repair-cost');

        let isValid = true;

        // Validar cliente
        if (typeof Validators !== 'undefined') {
            if (!Validators.string(customerInput?.value, { required: true }).valid) {
                isValid = false;
                if (customerInput) {
                    customerInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El cliente es requerido';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    customerInput.parentNode.insertBefore(error, customerInput.nextSibling);
                }
            }

            // Validar descripción
            if (!Validators.string(descriptionInput?.value, { required: true, minLength: 5, maxLength: 500 }).valid) {
                isValid = false;
                if (descriptionInput) {
                    descriptionInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'La descripción es requerida y debe tener entre 5 y 500 caracteres';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    descriptionInput.parentNode.insertBefore(error, descriptionInput.nextSibling);
                }
            }
            ...
            // Validar costo
            const costResult = Validators.number(costInput?.value, { required: false, min: 0 });
            if (!costResult.valid) {
                isValid = false;
                if (costInput) {
        ...
        const existingRepair = isEdit ? await DB.get('repairs', repairId) : null;
        
        if (isEdit && !existingRepair) {
            Utils.showNotification('La reparación que intentas editar no existe', 'error');
            return;
        }
        
        // Generar folio si es nuevo
        const folio = existingRepair?.folio || `REP-${Utils.formatDate(new Date(), 'YYYYMMDD')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;

        const repair = {
            // Solo incluir ID si es edición
            ...(isEdit ? { id: repairId } : {}),
            inventory_item_id: document.getElementById('repair-item')?.value || null,
            customer_name: customerNameInput?.value?.trim() || null,
            customer_phone: document.getElementById('repair-customer-phone')?.value?.trim() || null,
            customer_email: document.getElementById('repair-customer-email')?.value?.trim() || null,
            description: descriptionInput?.value?.trim() || '',
            status: statusInput?.value || 'recibida',
            cost: typeof Validators !== 'undefined' && costResult?.value !== undefined ? costResult.value : parseFloat(costInput?.value || 0)
        };

        try {
            let savedRepair;
            if (isEdit) {
                savedRepair = await DB.put('repairs', repair);
            } else {
                savedRepair = await DB.add('repairs', repair);
            }
            ...
            const finalRepairId = savedRepair.id || (isEdit ? repairId : null);
            if (!finalRepairId) {
                throw new Error('No se recibió ID de la reparación guardada');
            }
            ...
            let branchId = savedRepair.branch_id || null;
            let inventoryItemUpdated = false;
            if (repair.inventory_item_id) {
                try {
                    const item = await DB.get('inventory_items', repair.inventory_item_id);
                    if (item) {
                    ...
                    console.warn('Error actualizando estado de inventario:', itemError);
                }
            }
            ...
            if (typeof Costs !== 'undefined' && savedRepair.cost > 0 && savedRepair.status === 'completada') {
                try {
                    ...
                    if (!wasCompleted) {
                        await Costs.registerCost({
                            amount: savedRepair.cost,
                            category: 'reparacion',
                            type: 'variable',
                            description: `Costo de reparación - ${savedRepair.folio || 'N/A'}`,
                            date: new Date().toISOString().split('T')[0],
                            branch_id: branchId,
                            notes: `Reparación ${savedRepair.folio || 'N/A'}: ${savedRepair.description || ''}`,
                            repair_id: finalRepairId
                        });
                        ...
                        if (typeof Utils !== 'undefined' && Utils.EventBus) {
                            Utils.EventBus.emit('repair-completed', {
                                repair: savedRepair,
                                branchId: branchId,
                                cost: savedRepair.cost
                            });
                        }
                    }
                } catch (costError) {
                    console.warn('Error registrando costo de reparación:', costError);
                }
            }

            Utils.showNotification(isEdit ? 'Reparación actualizada' : 'Reparación creada', 'success');
            UI.closeModal();
            this.loadRepairs();
        } catch (error) {
            console.error('Error guardando reparación:', error);
            const errorMessage = error.message || error.error || 'Error desconocido';
            Utils.showNotification('Error al guardar reparación: ' + errorMessage, 'error');
        }
    },
