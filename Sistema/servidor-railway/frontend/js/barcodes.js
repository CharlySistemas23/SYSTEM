...
const BarcodeManager = {
    ...
    setupNavigationProtection() {
        ...
        window.addEventListener('beforeunload', (e) => {
            // ELIMINADObarcodes.js:7',message:'Beforeunload event',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        });
        
        // Interceptar cambios de hash (navegación interna)
        window.addEventListener('hashchange', (e) => {
            // ELIMINADObarcodes.js:13',message:'Hash change detected',data:{oldURL:e.oldURL,newURL:e.newURL},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        });
    },
    ...
    setupGlobalProtection() {
        ...
        document.addEventListener('submit', (e) => {
            // ELIMINADObarcodes.js:14',message:'Form submit detected',data:{formId:e.target.id,formAction:e.target.action,formMethod:e.target.method},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        }, true);
        ...
        document.addEventListener('keydown', (e) => {
            ...
            // ELIMINADObarcodes.js:25',message:'Global protection keydown',data:{key:e.key,ctrlKey:e.ctrlKey,rapidKeyCount:rapidKeyCount,timeSinceLastKey:timeSinceLastKey},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            
            // Si hay escaneo rápido activo, prevenir atajos del navegador
            if (rapidKeyCount > 3) {
                // Prevenir atajos comunes que pueden abrir ventanas
                if (e.ctrlKey || e.metaKey) {
        // ELIMINADObarcodes.js:32',message:'Rapid scan - blocking Ctrl/Meta',data:{key:e.key,rapidKeyCount:rapidKeyCount},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
    ...
    setupScanner() {
        ...
        const scanner = Utils.createBarcodeScanner((barcode) => {
            // ELIMINADObarcodes.js:55',message:'Scanner callback triggered',data:{barcode:barcode,isScanning:isScanning},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        ...
        // ELIMINADObarcodes.js:69',message:'Temp listener blocking event',data:{key:e.key,ctrlKey:e.ctrlKey,metaKey:e.metaKey,altKey:e.altKey,type:e.type},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                    preventDefault(e);
                }
            };
        ...
        // ELIMINADObarcodes.js:92',message:'Blocking event during scan',data:{key:e.key},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
            ...
            if (e.ctrlKey || e.metaKey) {
        // ELIMINADObarcodes.js:115',message:'Ctrl/Meta key detected',data:{key:e.key,ctrlKey:e.ctrlKey,metaKey:e.metaKey},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                ...
                e.stopPropagation();
                e.stopImmediatePropagation();
        // ELIMINADObarcodes.js:123',message:'Blocked Ctrl/Meta shortcut',data:{key:e.key,prevented:e.defaultPrevented},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                return false;
            }
