...
const Settings = {
    ...
    async init() {
        ...
        // Verificar que los catálogos estén cargados antes de inicializar (Settings gestiona catálogos)
        let catalogsLoaded = false;
        let attempts = 0;
        const maxAttempts = 10; // Máximo 5 segundos de espera
        
        while (!catalogsLoaded && attempts < maxAttempts) {
            try {
                ...
                // Si hay al menos algunos datos, considerar que están cargados
                // (Settings puede funcionar sin catálogos, pero es mejor esperarlos)
                if (branches.length > 0 || sellers.length > 0 || guides.length > 0 || agencies.length > 0) {
                    catalogsLoaded = true;
                    break;
                }
                ...
                if (attempts < maxAttempts - 1) {
                    ...
                    // Último intento: continuar de todas formas (Settings puede funcionar sin catálogos)
                    console.log('⚠️ Catálogos no cargados en Settings, continuando...');
                    catalogsLoaded = true;
                }
            } catch (e) {
                console.error('Error verificando catálogos en Settings:', e);
                catalogsLoaded = true;
                break;
            }
        }
        ...
        return `
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md);">
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light);">
                    ...
                    <button class="btn-secondary btn-sm" disabled style="margin-top: 5px; width: 100%; opacity: 0.5;">
                        <i class="fas fa-key"></i> Sincronización con Sheets deshabilitada (Backend centralizado)
                    </button>
                    <div class="form-group">
    ...
    async clearDatabase() {
        ...
        try {
            // Ya no se usa IndexedDB - el backend es centralizado
            // Los datos se eliminan desde el backend si es necesario
        ...
        const body = `
            <div style="max-width: 1000px;">
                <div style="margin-bottom: var(--spacing-md); padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 11px; color: var(--color-text-secondary);">
                    <strong>ℹ️ Información:</strong> Los Administradores tienen acceso total automáticamente. Otros roles tienen permisos predefinidos que pueden personalizarse.
                </div>
                <div style="max-height: 500px; overflow-y: auto;">
                            ...
                            ${users.map(user => {
                                ...
                                return `
                                    <tr>
                                        ...
                                        <td>
                                            ${!isAdmin ? `
                                                <button class="btn-secondary btn-sm" onclick="window.Settings.editUserPermissions('${user.id}')" title="Ver y editar permisos">
                                                    <i class="fas fa-edit"></i> Editar
                                                </button>
    ...
    async editUserPermissions(userId) {
        ...
        // Admin no necesita configuración de permisos
        if (user.role === 'admin') {
            Utils.showNotification('Los administradores tienen acceso total y no necesitan configuración de permisos', 'info');
            return;
        }

        // Asegurar que el usuario tenga permisos según su rol si no tiene personalizados
        if (typeof PermissionManager !== 'undefined') {
            await PermissionManager.ensureUserPermissions(user);
        }

        const userPermissions = Array.isArray(user.permissions) ? user.permissions : [];
        const allPermissions = PermissionManager.getAllPermissions();
        const grouped = PermissionManager.groupPermissionsByCategory(allPermissions);
        
        // Mapear nombres legibles de permisos
        const permissionLabels = {
            // OPERACIONES
            'pos.view': 'Ver POS',
            'pos.create_sale': 'Crear Ventas',
            'pos.edit_sale': 'Editar Ventas',
            'pos.cancel_sale': 'Cancelar Ventas',
            'pos.apply_discount': 'Aplicar Descuentos',
            'cash.view': 'Ver Caja',
            'cash.open_session': 'Abrir Sesión de Caja',
            'cash.close_session': 'Cerrar Sesión de Caja',
            'cash.view_reports': 'Ver Reportes de Caja',
            'barcodes.generate': 'Generar Códigos de Barras',
            'barcodes.print': 'Imprimir Códigos de Barras',
            // INVENTARIO
            'inventory.view': 'Ver Inventario',
            'inventory.add': 'Agregar Items',
            'inventory.edit': 'Editar Items',
            'inventory.delete': 'Eliminar Items',
            'inventory.update_stock': 'Actualizar Stock',
            'inventory.view_cost': 'Ver Costos',
            'inventory.edit_cost': 'Editar Costos',
            'transfers.view': 'Ver Transferencias',
            'transfers.create': 'Crear Transferencias',
            'transfers.approve': 'Aprobar Transferencias',
            // CLIENTES Y SERVICIOS
            'customers.view': 'Ver Clientes',
            'customers.add': 'Agregar Clientes',
            'customers.edit': 'Editar Clientes',
            'customers.delete': 'Eliminar Clientes',
            'repairs.view': 'Ver Reparaciones',
            'repairs.create': 'Crear Reparaciones',
            'repairs.edit': 'Editar Reparaciones',
            'repairs.complete': 'Completar Reparaciones',
            'arrivals.view': 'Ver Llegadas',
            'arrivals.register': 'Registrar Llegadas',
            'arrivals.edit': 'Editar Llegadas',
            // ADMINISTRACIÓN
            'employees.view': 'Ver Empleados',
            'employees.add': 'Agregar Empleados',
            'employees.edit': 'Editar Empleados',
            'employees.delete': 'Eliminar Empleados',
            'employees.view_users': 'Ver Usuarios',
            'employees.create_users': 'Crear Usuarios',
            'employees.edit_users': 'Editar Usuarios',
            'employees.reset_pin': 'Restablecer PINs',
            ...
            'reports.view_profits': 'Ver Utilidades',
            'reports.view_costs': 'Ver Costos en Reportes',
            'reports.view_analytics': 'Ver Análisis Avanzados',
            'costs.view': 'Ver Módulo Costos',
            'costs.add': 'Agregar Costos',
            'costs.edit': 'Editar Costos',
            'costs.delete': 'Eliminar Costos',
            'dashboard.view': 'Ver Dashboard',
            'dashboard.view_all_branches': 'Ver Todas las Sucursales en Dashboard',
            // CONFIGURACIÓN
            'settings.view': 'Ver Configuración',
            'settings.edit_general': 'Editar Configuración General',
            'settings.edit_financial': 'Editar Configuración Financiera',
            'settings.manage_catalogs': 'Gestionar Catálogos',
            'settings.manage_permissions': 'Gestionar Permisos',
            'settings.view_audit': 'Ver Auditoría',
            'settings.sync': 'Sincronizar Datos',
            'settings.qa': 'Acceso a QA/Autopruebas'
        };

        const categoryLabels = {
            operations: 'OPERACIONES',
            inventory: 'INVENTARIO',
            customers_services: 'CLIENTES Y SERVICIOS',
            administration: 'ADMINISTRACIÓN',
            reports_analysis: 'REPORTES Y ANÁLISIS',
            configuration: 'CONFIGURACIÓN'
        };

        const renderCategory = (categoryName, perms) => {
            if (!perms || perms.length === 0) return '';
            return `
                <div style="margin-bottom: var(--spacing-lg);">
                    <h4 style="font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: var(--spacing-sm); color: var(--color-primary);">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-sm);">
                        ${perms.map(perm => {
                            ...
                            const label = permissionLabels[perm] || perm.replace(/\./g, ' ').replace(/\b\w/g, l => l.toUpperCase());
                            return `
                                <label style="display: flex; align-items: center; gap: var(--spacing-xs); cursor: pointer; padding: var(--spacing-xs); border-radius: var(--radius-xs); transition: background 0.2s;" 
                                       onmouseover="this.style.background='var(--color-bg-secondary)'" 
                                       onmouseout="this.style.background='transparent'">
                                    <input type="checkbox" value="${perm}" ${isChecked ? 'checked' : ''} style="cursor: pointer;">
        ...
        const body = `
            <form id="permissions-form" style="max-width: 800px; max-height: 600px; overflow-y: auto;">
                <div style="margin-bottom: var(--spacing-md); padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm);">
                    <strong>Usuario:</strong> ${user.username}<br>
                    <strong>Rol:</strong> ${user.role}<br>
                    <button type="button" class="btn-secondary btn-sm" onclick="window.Settings.resetToRoleProfile('${userId}')" style="margin-top: var(--spacing-xs);">
                        Restablecer a Perfil de Rol
                    </button>
                </div>
                ${renderCategory('operations', grouped.operations)}
                ${renderCategory('inventory', grouped.inventory)}
                ${renderCategory('customers_services', grouped.customers_services)}
                ${renderCategory('administration', grouped.administration)}
                ${renderCategory('reports_analysis', grouped.reports_analysis)}
                ${renderCategory('configuration', grouped.configuration)}
            </form>
        `;
    ...
    async resetToRoleProfile(userId) {
        ...
        Utils.showNotification('Permisos restablecidos al perfil del rol', 'success');
        
        // Recargar el modal de edición
        await this.editUserPermissions(userId);
    },
    ...
    async testSyncConnection() {
        // Ya no se necesita sincronización con Sheets - el backend es centralizado y tiempo real
        Utils.showNotification('La sincronización con Google Sheets ya no es necesaria. El sistema usa un backend centralizado en tiempo real.', 'info');
    },

    async forceSync() {
        // Ya no se necesita sincronización con Sheets - el backend es centralizado y tiempo real
        Utils.showNotification('La sincronización ya no es necesaria. El sistema usa un backend centralizado en tiempo real.', 'info');
    },
    ...
    async loadSyncStatus() {
        ...
        try {
            ...
            // Ya no se usa SyncManager - el backend es centralizado y tiempo real
            // Verificar autenticación de Google directamente si es necesario
            try {
            ...
            // Ya no hay sync_logs - el backend es centralizado y tiempo real
            // Todos los datos se sincronizan automáticamente con WebSockets
            const lastSync = null;

            // Ya no hay sync_queue - el backend es centralizado y tiempo real
            // Todos los datos se sincronizan automáticamente con WebSockets
            // El código anterior de sync_queue ha sido eliminado

            // Ya no hay sync_queue - el backend es centralizado y tiempo real
            // Si no está autenticado y no hay logs, mostrar "Backend centralizado"
            if (!isAuthenticated && !lastSync) {
                statusContainer.innerHTML = `
                    <div style="margin-bottom: var(--spacing-xs);">
                        <strong>Estado:</strong> 
                        <span style="color: var(--color-text-secondary);">Backend centralizado</span>
                    </div>
                    <div style="font-size: 10px; color: var(--color-text-secondary);">
    ...
    async loadDemoData() {
        ...
        try {
            // ELIMINADOsettings.js:1889',message:'loadDemoData START',data:{timestamp:new Date().toISOString()},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});

            // Verificar que los módulos necesarios estén disponibles
            if (typeof ArrivalRules === 'undefined') {
        // ELIMINADOsettings.js:1896',message:'ArrivalRules undefined',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
            ...
            // ELIMINADOsettings.js:1906',message:'Parameters initialized',data:{branchId,today:today.toISOString()},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            ...
            for (let i = 0; i < 2; i++) {
                ...
                if (typeof Costs !== 'undefined' && Costs.normalizeCostData) {
                    normalizedCost = Costs.normalizeCostData(costData);
                } else {
                    normalizedCost = {
                        id: costData.id,
                        type: costData.type,
                ...
                await DB.put('cost_entries', normalizedCost);
                // Ya no se necesita SyncManager - el backend es centralizado y tiempo real
                actions.push(`✓ Costo normal ${i + 1} creado`);
            }

            // 6. Productos de inventario (5 productos)
            // ELIMINADOsettings.js:2105',message:'Starting inventory items generation',data:{count:5},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        ...
        // ELIMINADOsettings.js:2120',message:'Inventory item saved',data:{id:inventoryItem.id,sku:inventoryItem.sku},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                    // Ya no se necesita SyncManager - el backend es centralizado y tiempo real
                    actions.push(`✓ Producto ${i + 1} (${inventoryItem.name}) creado`);
                } catch (e) {
        // ELIMINADOsettings.js:2125',message:'Error creating inventory item',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating inventory item ${i + 1}:`, e);
                }
            }

            // 7. Clientes (3 clientes)
            // ELIMINADOsettings.js:2130',message:'Starting customers generation',data:{count:3},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        ...
        // ELIMINADOsettings.js:2145',message:'Customer saved',data:{id:customer.id,name:customer.name},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                    // Ya no se necesita SyncManager - el backend es centralizado y tiempo real
                    actions.push(`✓ Cliente ${i + 1} (${customer.name}) creado`);
                } catch (e) {
        // ELIMINADOsettings.js:2150',message:'Error creating customer',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating customer ${i + 1}:`, e);
                }
            }

            // 8. Reparaciones (2 reparaciones)
            // ELIMINADOsettings.js:2155',message:'Starting repairs generation',data:{count:2},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        ...
        // ELIMINADOsettings.js:2175',message:'Error creating repair',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating repair ${i + 1}:`, e);
                }
            }

            // 9. Ventas relacionadas (3 ventas con pagos e items)
            // ELIMINADOsettings.js:2180',message:'Starting sales generation',data:{count:3},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            const sellers = await DB.getAll('catalog_sellers') || [];
            const guides = await DB.getAll('catalog_guides') || [];
        ...
        // ELIMINADOsettings.js:2215',message:'Sale saved',data:{id:sale.id,folio:sale.folio,total:sale.total},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                    // Ya no se necesita SyncManager - el backend es centralizado y tiempo real
        ...
        // ELIMINADOsettings.js:2235',message:'Error creating sale',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating sale ${i + 1}:`, e);
                }
            }

            // 10. Reportes turistas completos (2 reportes con líneas)
            // ELIMINADOsettings.js:2240',message:'Starting tourist reports generation',data:{count:2},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            const allSales = await DB.getAll('sales') || [];
        ...
        // ELIMINADOsettings.js:2290',message:'Error creating tourist report',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating tourist report ${i + 1}:`, e);
                }
            }

            // 11. Sesiones de caja (2 sesiones)
            // ELIMINADOsettings.js:2295',message:'Starting cash sessions generation',data:{count:2},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            const users = await DB.getAll('users') || [];
        ...
        // ELIMINADOsettings.js:2335',message:'Error creating cash session',data:{index:i,error:e.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    console.error(`Error creating cash session ${i + 1}:`, e);
                }
            }
            
            // ELIMINADOsettings.js:2340',message:'loadDemoData COMPLETE',data:{totalActions:actions.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            ...
            // ELIMINADOsettings.js:2509',message:'Refreshing modules',data:{currentModule:localStorage.getItem('current_module')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        ...
        // ELIMINADOsettings.js:2535',message:'Error refreshing modules',data:{error:refreshError.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            ...
            // ELIMINADOsettings.js:2455',message:'Error in loadDemoData',data:{error:e.message,stack:e.stack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
    ...
    async clearMockData() {
        ...
        try {
            ...
            // Ya no hay IndexedDB - el backend es centralizado
            // Solo podemos limpiar datos locales (localStorage) para stores locales
            // Los datos del backend no se pueden limpiar desde el frontend por seguridad
            ...
            // Solo limpiar stores locales (localStorage) - los datos del backend no se pueden limpiar desde aquí
            // Stores locales según db.js: 'settings', 'device', 'audit_log', etc.
            // sync_queue, sync_logs, sync_deleted_items ELIMINADOS - ya no hay sincronización local
            const localStores = ['audit_log', 'barcode_scan_history', 
                                'barcode_print_templates', 'qa_test_runs', 'qa_coverage', 'qa_errors', 'qa_fixes', 
                                'inventory_logs', 'inventory_price_history'];
            
            for (const storeName of storesToClearFiltered) {
                try {
                    // Solo limpiar si es un store local
                    if (localStores.includes(storeName)) {
                        // Limpiar desde localStorage (db.js usa localStorage para stores locales)
                        const prefix = `db_${storeName}_`;
                        const keysToRemove = [];
                        ...
                        clearedCount++;
                        console.log(`✅ Limpiado (local): ${storeName} (${keysToRemove.length} items)`);
                    } else {
                        // Store del backend - no se puede limpiar desde el frontend
                        console.warn(`⚠️ Store ${storeName} está en el backend y no se puede limpiar desde el frontend`);
                    }
                } catch (error) {
                    errorCount++;
            ...
            if (storesToClearFiltered.length > clearedCount) {
                Utils.showNotification(
                    `Limpieza completada: ${clearedCount} stores locales limpiados. ` +
                    `Nota: Los datos del backend centralizado no se pueden limpiar desde el frontend por seguridad.`,
                    'info'
                );
            }
