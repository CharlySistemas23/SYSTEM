...
const Cash = {
    ...
    async init() {
        ...
        while (!catalogsLoaded && attempts < maxAttempts) {
            try {
                ...
                if (branches.length > 0 || agencies.length > 0) {
                    catalogsLoaded = true;
                    break;
                }
                ...
                if (attempts < maxAttempts - 1) {
                    ...
                    console.log('⚠️ Catálogos no cargados en Cash, continuando...');
                    catalogsLoaded = true;
                }
            } catch (e) {
                console.error('Error verificando catálogos en Cash:', e);
                catalogsLoaded = true;
                break;
            }
        }
        ...
        this.setupUI();
        // Esperar a que el DOM se actualice antes de cargar la sesión
    ...
    setupWebSocketListeners() {
        ...
        SocketManager.on('cash-session-created', (data) => {
            if (this.shouldShowSession(data)) {
                this.loadCurrentSession();
                this.loadHistory();
            }
        });
        
        SocketManager.on('cash-session-updated', (data) => {
            if (this.shouldShowSession(data)) {
                this.loadCurrentSession();
                this.loadHistory();
            }
    ...
    // Verificar si una sesión debe mostrarse según el filtro de sucursal activo
    shouldShowSession(session) {
        if (typeof BranchSelector !== 'undefined' && BranchSelector.isAdmin && BranchSelector.selectedBranchId) {
            return session.branch_id === BranchSelector.selectedBranchId;
        }
        return true;
    },
    ...
    async loadCurrentSession() {
        try {
            ...
            // Buscar sesión abierta del día de hoy
            ...
            Utils.showNotification('Error al cargar sesión de caja', 'error');
        }
    },
    ...
    async loadHistory() {
        try {
            const branchId = await this.getCurrentBranchId();
            let sessions = await DB.query('cash_sessions', 'branch_id', branchId);
