...
const BackupManager = {
    ...
    async exportAllData() {
        ...
        const knownStores = [
            'employees', 'users', 'inventory_items', 'sales', 'sale_items', 'payments', 'customers', 
            ...
            'agency_arrivals', 'inventory_transfers', 'tourist_reports', 'tourist_report_lines', 'cost_entries',
            'payment_methods', 'commission_rules'
        ];
        
        // Stores que solo usan localStorage (no tienen endpoints en el backend)
        const localOnlyStores = ['settings', 'device', 'audit_log', 'barcode_scan_history', 
                                 'barcode_print_templates', 'qa_test_runs', 'qa_coverage', 
                                 'qa_errors', 'qa_fixes', 'inventory_logs', 'inventory_price_history'];
        
        for (const store of stores) {
            try {
                // Si es store local, obtener solo de localStorage
                if (localOnlyStores.includes(store)) {
                    try {
                        const localData = localStorage.getItem(`store_${store}`) || localStorage.getItem(`db_${store}_default`);
                        exportData.stores[store] = localData ? JSON.parse(localData) : [];
                    } catch (e) {
                        exportData.stores[store] = [];
                    }
                    continue; // Continuar con el siguiente store
                }
                
                // Verificar que el store está en la lista de conocidos o intentar obtenerlo de todas formas
                if (!knownStores.includes(store)) {
                    // Solo mostrar warning si realmente intentamos obtenerlo de la API
                ...
                // Si el error es 404 o "no encontrado", simplemente retornar array vacío sin loguear
