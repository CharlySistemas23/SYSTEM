...
const Costs = {
    ...
    // Normalizar datos de costo antes de enviar al backend
    // Remueve campos no soportados y valida campos requeridos
    normalizeCostData(costData) {
        // Validar campos requeridos
        if (!costData.type) {
            throw new Error('El campo "type" es requerido');
        }
        if (costData.amount === undefined || costData.amount === null || costData.amount === '') {
        ...
        // Obtener branch_id (del objeto o del BranchManager)
        let branchId = costData.branch_id;
        if (!branchId && typeof BranchManager !== 'undefined') {
            branchId = BranchManager.getCurrentBranchId();
        }
        if (branchId === '' || branchId === null) {
            branchId = null; // Permitir null para costos globales
        }
        
        // Crear objeto normalizado con solo los campos soportados por el backend
        const normalized = {
            id: costData.id || Utils.generateId(),
            type: String(costData.type).trim(),
            category: costData.category ? String(costData.category).trim() : null,
            amount: parseFloat(costData.amount),
            date: String(costData.date).trim(),
            branch_id: branchId,
            notes: costData.notes ? String(costData.notes).trim() : null
        };
        
        // Preservar created_at si existe (para actualizaciones)
        if (costData.created_at) {
            normalized.created_at = costData.created_at;
        }
    ...
    async init() {
        ...
        // Verificar que los catálogos estén cargados antes de inicializar
        ...
        while (!catalogsLoaded && attempts < maxAttempts) {
            try {
                ...
                if (attempts < maxAttempts - 1) {
                    ...
                    // Último intento: continuar de todas formas
                    console.log('⚠️ Catálogos no cargados en Costs, continuando...');
                    catalogsLoaded = true;
                }
            } catch (e) {
                console.error('Error verificando catálogos en Costs:', e);
                catalogsLoaded = true;
                break;
            }
        }
            ...
            for (const cost of recurringCosts) {
                ...
                if (cost.period_type === 'monthly') {
                    ...
                    if (!existing || (existing && new Date(existing.date || existing.created_at).getMonth() !== currentMonth)) {
                        ...
                        const newCost = this.normalizeCostData(newCostData);
    ...
    async loadCosts() {
        ...
        try {
        // Obtener sucursal seleccionada en el filtro
        ...
        const selectedBranchId = branchFilterValue === '' ? null : branchFilterValue;
            ...
            let costs = await this.getFilteredCosts({
    ...
    async saveCost(costId) {
        ...
        // Validar campos requeridos
        const typeInput = document.getElementById('cost-type');
        ...
        // Validar tipo
        if (typeof Validators !== 'undefined') {
            if (!Validators.string(typeInput?.value, { required: true }).valid) {
                isValid = false;
                if (typeInput) {
                    typeInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El tipo de costo es requerido';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    typeInput.parentNode.insertBefore(error, typeInput.nextSibling);
                }
            }

            // Validar monto
            const amountResult = Validators.number(amountInput?.value, { required: true, min: 0 });
            if (!amountResult.valid) {
                isValid = false;
                if (amountInput) {
                    amountInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = amountResult.error || 'El monto debe ser un número mayor o igual a 0';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    amountInput.parentNode.insertBefore(error, amountInput.nextSibling);
                }
            }

            // Validar fecha
            const dateResult = Validators.date(dateInput?.value, { required: true });
            if (!dateResult.valid) {
                isValid = false;
                if (dateInput) {
                    dateInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = dateResult.error || 'La fecha es requerida';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
        ...
        const isEdit = !!costId;
        const existingCost = isEdit ? await DB.get('cost_entries', costId) : null;
        
        if (isEdit && !existingCost) {
            Utils.showNotification('El costo que intentas editar no existe', 'error');
            return;
        }
        ...
        const costData = {
            ...
            category: document.getElementById('cost-category')?.value || null,
            amount: typeof Validators !== 'undefined' && amountResult?.value !== undefined ? amountResult.value : parseFloat(amountInput?.value || 0),
            branch_id: branchId,
        ...
        try {
            ...
            const normalizedCost = this.normalizeCostData(costData);
            
            let savedCost;
            if (isEdit) {
                savedCost = await DB.put('cost_entries', normalizedCost);
            } else {
                savedCost = await DB.add('cost_entries', normalizedCost);
            }
            
            if (!savedCost) {
                throw new Error('No se recibió respuesta del servidor');
            }
            Utils.showNotification(isEdit ? 'Costo actualizado' : 'Costo creado', 'success');
            UI.closeModal();
            ...
            Utils.showNotification('Error al guardar el costo: ' + errorMessage, 'error');
        }
    },
    ...
    async deleteCost(costId) {
        ...
        try {
            ...
            // Eliminar el costo de la base de datos
            try {
    ...
    async registerArrivalPayment(arrivalId, amount, branchId, agencyId, passengers) {
