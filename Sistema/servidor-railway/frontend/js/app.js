...
const App = {
    ...
    async init() {
        ...
        // Solo si no hay sesión activa (no mostrar código si ya está logueado)
        const hasActiveSession = typeof API !== 'undefined' && API.token;
        if (!hasActiveSession) {
            await this.initCompanyCodeAccess();
        }
        
        // Las pantallas de login se manejan en UserManager.init() y autoLogin()
        
        try {
            // Initialize database (ahora solo inicializa el wrapper del backend)
            await DB.init();
            console.log('✅ Database Manager inicializado (usando Backend API)');
            ...
            // Sync Manager YA NO SE NECESITA - el backend es centralizado y tiempo real
            // await SyncManager.init(); // ELIMINADO - ya no hay sincronización con Sheets

            // Initialize User Manager
            await UserManager.init();
            console.log('User manager initialized');

            // Initialize Branch Manager si hay usuario (autenticado o simulado)
            // Si no hay token, BranchManager se inicializará después del login exitoso
            if (typeof BranchManager !== 'undefined' && (typeof API !== 'undefined' && API.token || UserManager.currentUser)) {
                try {
                    ...
                    // Si hay error inicializando BranchManager (ej: no hay token), ignorar silenciosamente
                    // Se inicializará después del login exitoso
                    console.log('BranchManager no inicializado aún (esperando login):', e.message);
                }
            } else {
                console.log('BranchManager se inicializará después del login exitoso');
            }
            
            // Initialize Branch Selector (selector de sucursal para admin)
            // Solo inicializar si hay token o usuario simulado (se inicializará después del login si no hay token)
            if (typeof BranchSelector !== 'undefined' && BranchSelector.init && (typeof API !== 'undefined' && API.token || UserManager.currentUser)) {
                await BranchSelector.init();
                console.log('Branch selector initialized');
            }
            
            // Initialize Backup Manager (backups automáticos cada 10 minutos)
            // Solo inicializar si hay token o usuario simulado (se inicializará después del login si no hay token)
            if (typeof BackupManager !== 'undefined' && (typeof API !== 'undefined' && API.token || UserManager.currentUser)) {
            ...
            // Solo inicializar si hay token o usuario simulado (se inicializará después del login si no hay token)
            if (typeof ExchangeRates !== 'undefined' && (typeof API !== 'undefined' && API.token || UserManager.currentUser)) {
                await ExchangeRates.init();
                console.log('Exchange rates manager initialized');
            }
            
            // FUNCIONES OBSOLETAS ELIMINADAS:
            // - createDemoUsers: Ya no se necesitan usuarios demo locales
            // - Auto-create users: El backend tiene seed automático que crea usuarios al iniciar
            // Los usuarios se crean desde el backend (seed.js) o desde el admin del sistema
...
// Debug code removed - DB ya no tiene .db (usando API ahora)
                console.log('=== BYPASS LOGIN ===');
                try {
                    // El backend ya tiene usuarios creados por seed.js
                    // Simplemente obtenerlos del servidor
                    console.log('Obteniendo usuarios del servidor...');
                    let users = [];
                    try {
                        if (typeof API !== 'undefined' && API.getUsers) {
                            users = await API.getUsers();
                        } else {
                            users = await DB.getAll('users') || [];
                        }
                    } catch (e) {
                        console.warn('No se pudieron obtener usuarios del servidor:', e);
                        users = [];
                    }

                    // Si no hay usuarios en el servidor, usar login normal
                    if (!Array.isArray(users) || users.length === 0) {
                        console.warn('No hay usuarios en el servidor. Usa login normal con: admin / admin123');
                        if (Utils && Utils.showNotification) {
                            Utils.showNotification('No hay usuarios. Usa login normal: admin / admin123', 'warning');
                        }
                        return;
                    }
            ...
            // FUNCIONES OBSOLETAS ELIMINADAS:
            // - createUsersManually: Ya no se necesita - el backend tiene seed.js que crea usuarios automáticamente
            // - createDemoUsers: Ya no se necesita - usar seed del backend
            // Los usuarios se crean desde el backend (seed.js) o desde el admin del sistema
            ...
            // Sync button ELIMINADO - Ya no hay sincronización con Sheets
            // El sistema es centralizado y tiempo real con WebSockets
            ...
            // Cargar datos básicos del sistema si hay token o usuario simulado
            // CRÍTICO: Esperar datos críticos antes de continuar (catálogos)
            if ((typeof API !== 'undefined' && API.token) || UserManager.currentUser) {
                try {
                    // Cargar solo datos críticos primero (catálogos)
                    await this.loadSystemData(true); // criticalOnly = true
                    console.log('✅ Datos críticos cargados, sistema listo');
                    
                    // Cargar datos no críticos en background después
                    this.loadSystemData(false).catch(e => {
                        console.error('Error cargando datos no críticos:', e);
                    });
                } catch (e) {
                    console.error('❌ Error cargando datos críticos:', e);
                    Utils.showNotification('Error cargando datos del sistema. Algunas funciones pueden no estar disponibles.', 'warning');
                    // Continuar de todas formas para que el usuario pueda ver la UI
                }
            } else {
                console.log('loadSystemData: No hay token, omitiendo carga hasta después del login');
            }
            ...
            // Solo si hay token o usuario simulado (se ejecutará después del login)
            if (typeof Costs !== 'undefined' && Costs.generateAllWeeklyPayrolls && ((typeof API !== 'undefined' && API.token) || UserManager.currentUser)) {
            ...
            } else if (typeof Costs !== 'undefined' && Costs.generateAllWeeklyPayrolls) {
                console.log('generateAllWeeklyPayrolls: No hay token, omitiendo hasta después del login');
            }
            
            // Verificar y corregir códigos de barras faltantes o inconsistentes
            // Solo si hay token o usuario simulado (ejecutar en background, no bloquea)
            if ((typeof API !== 'undefined' && API.token) || UserManager.currentUser) {
                // Ejecutar después de 3 segundos en background (no bloquea carga inicial)
                setTimeout(() => {
                    this.verifyAndFixBarcodes().catch(e => {
                        console.error('Error verificando códigos de barras:', e);
                    });
                }, 3000);
            } else {
                console.log('verifyAndFixBarcodes: No hay token, omitiendo hasta después del login');
            }
            ...
            if (UserManager.currentUser) {
                ...
                if (typeof API !== 'undefined') {
                    API.currentUser = UserManager.currentUser;
                }
                
                // Inicializar BranchManager si no se inicializó antes
                if (typeof BranchManager !== 'undefined' && !BranchManager.currentBranchId) {
                    try {
                        await BranchManager.init();
                        console.log('Branch manager initialized (sin login)');
                    } catch (e) {
                ...
                // Mostrar dashboard
                const savedModule = localStorage.getItem('current_module') || 'dashboard';
                if (UI && UI.showModule) {
                    setTimeout(async () => {
                        await UI.showModule(savedModule);
                    }, 100);
                }
            }
        } catch (e) {
        ...
        try {
            ...
            switch (moduleName) {
                ...
                case 'pos':
                    ...
                    break;
                case 'inventory':
// ELIMINADO458',message:'Loading inventory module',data:{inventoryDefined:typeof Inventory!=='undefined',initialized:typeof Inventory!=='undefined'?Inventory.initialized:null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'F'})}).catch(()=>{});
                    if (abortController.aborted) return;
                ...
                case 'employees':
                    if (abortController.aborted) return;
// ELIMINADO508',message:'Loading employees module',data:{employeesDefined:typeof Employees!=='undefined',initialized:typeof Employees!=='undefined'?Employees.initialized:false},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                    if (typeof Employees !== 'undefined') {
                        if (!Employees.initialized) {
// ELIMINADO511',message:'Calling Employees.init()',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                            await Employees.init();
                            if (abortController.aborted) return;
// ELIMINADO513',message:'Employees.init() completed',data:{initialized:Employees.initialized},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                        } else {
                            if (abortController.aborted) return;
// ELIMINADO515',message:'Employees already initialized, checking UI',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                            // Verificar si el contenido necesita ser reconfigurado
                            const content = document.getElementById('module-content');
// ELIMINADO525',message:'Checking module-content for reconfig',data:{contentExists:!!content,innerHTMLLength:content?.innerHTML.length||0,innerHTMLPreview:content?.innerHTML.substring(0,100)||'N/A',hasEmployeesTabs:content?.querySelector('#employees-tabs')!==null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                            // Siempre reconfigurar si no tiene el elemento #employees-tabs
                            if (!content || !content.querySelector('#employees-tabs')) {
                                if (abortController.aborted) return;
// ELIMINADO530',message:'Reconfiguring UI via setupUI() - tabs missing',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                                Employees.setupUI();
                            }
                            if (abortController.aborted) return;
                            // Siempre recargar los datos cuando se navega al módulo
                            if (typeof Employees.loadEmployees === 'function') {
// ELIMINADO524',message:'Reloading employees data',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
                                await Employees.loadEmployees();
                            }
                        }
                    } else {
// ELIMINADO529',message:'Employees module not defined',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                    }
                    break;
                case 'settings':
                    if (abortController.aborted) return;
    ...
    async loadDashboard() {
        try {
            ...
            if (alertsList) {
                ...
                // Sync queue ELIMINADO - ya no hay sincronización con Sheets
                // El sistema es centralizado y tiempo real
    ...
    async loadSystemData(criticalOnly = false) {
        ...
        try {
            ...
            // Asegurar que DB esté inicializado (ahora solo verifica conexión al backend)
            if (!DB.initialized) {
                console.log('Inicializando DB Manager...');
                await DB.init();
            }
            
            // CRÍTICO: Cargar catálogos primero (necesarios para todos los módulos)
            try {
                await this.loadCatalogs();
                console.log('✅ Catálogos cargados (críticos)');
            } catch (e) {
                console.error('⚠️ Error cargando catálogos:', e);
                // Si falla cargar catálogos, el sistema no puede funcionar correctamente
                throw new Error('No se pudieron cargar los catálogos. El sistema no puede funcionar sin ellos.');
            }
            
            // Si solo se solicitan datos críticos, retornar aquí
            if (criticalOnly) {
                console.log('✅ Datos críticos cargados');
                return;
            }
            
            // Cargar datos no críticos en paralelo (después de catálogos)
            const loadPromises = [];
            
            // Precargar reglas de llegadas (arrival_rate_rules) - no crítico
            loadPromises.push(
                this.loadArrivalRateRules().catch(e => {
                console.warn('⚠️ Error cargando reglas de llegadas (no crítico):', e.message || e);
                })
            );
            
            // Precargar nómina semanal - no crítico
            loadPromises.push(
                this.loadWeeklyPayroll().catch(e => {
                console.warn('⚠️ Error cargando nómina semanal (no crítico):', e.message || e);
                })
            );
            
            // Precargar costos fijos y variables iniciales - no crítico
            loadPromises.push(
                this.loadInitialCosts().catch(e => {
                console.warn('⚠️ Error cargando costos iniciales (no crítico):', e.message || e);
                })
            );
            
            // Esperar todas las cargas no críticas en paralelo (no bloquean)
            await Promise.all(loadPromises);
            
            console.log('✅ Datos básicos del sistema cargados (con advertencias si hay tablas faltantes)');
        } catch (e) {
            // Error crítico solo si es algo que impide el funcionamiento básico
            const isTableError = e.message && (e.message.includes('no existe') || e.message.includes('does not exist') || e.message.includes('Tabla no existe'));
            
            if (isTableError) {
                console.error('❌ ERROR: Tablas faltantes detectadas. Intentando auto-reparación...');
                
                // Intentar auto-reparación automáticamente
                try {
                    const baseURL = typeof API !== 'undefined' && API.baseURL ? API.baseURL : '';
                    const token = typeof API !== 'undefined' && API.token ? API.token : localStorage.getItem('token');
                    
                    if (token) {
                        const repairResponse = await fetch(`${baseURL}/api/diagnose/auto-repair`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (repairResponse.ok) {
                            ...
                            console.log('✅ Auto-reparación ejecutada:', repairData);
                            Utils.showNotification('✅ Sistema reparado automáticamente. Recargando datos...', 'success');
                            
                            // Reintentar cargar datos después de un momento
                            setTimeout(() => {
                                this.loadSystemData(criticalOnly).catch(err => {
                                    console.error('Error después de reparación:', err);
                                });
                            }, 2000);
                ...
                console.error('❌ ERROR: Tablas faltantes en la base de datos. Ejecuta la migración desde /api/diagnose/migrate');
                Utils.showNotification('⚠️ Base de datos incompleta. El sistema intentó reparar automáticamente. Si el problema persiste, ejecuta la migración manualmente.', 'warning');
            } else {
    ...
    async loadCatalogs() {
        // Obtener branch_id actual (necesario para crear agencias/vendedores/guías)
        let branchId = null;
        ...
        if (!branchId) {
            try {
                const apiUser = localStorage.getItem('api_user');
                if (apiUser) {
        ...
        if (!branchId) {
            console.warn('⚠️ No hay branch_id configurado, omitiendo carga de catálogos');
            return;
        }

        // Agencies - Cargar en paralelo para mejor rendimiento
        const agencies = [
            { id: 'ag1', name: 'TRAVELEX', active: true, branch_id: branchId },
            { id: 'ag2', name: 'VERANOS', active: true, branch_id: branchId },
            { id: 'ag3', name: 'TANITOURS', active: true, branch_id: branchId },
            { id: 'ag4', name: 'DISCOVERY', active: true, branch_id: branchId },
            { id: 'ag5', name: 'TB', active: true, branch_id: branchId },
            { id: 'ag6', name: 'TTF', active: true, branch_id: branchId }
        ];

        // Procesar agencias en paralelo (mejor rendimiento)
        await Promise.all(agencies.map(async (agency) => {
            try {
                ...
                if (existing) {
                    ...
                    // Nuevo registro: generar código de barras y crear
                    agency.barcode = Utils.generateAgencyBarcode(agency);
                    await DB.add('catalog_agencies', agency);
                }
            } catch (e) {
                // Si el error es 404, intentar crear el registro
                if (e.message && e.message.includes('404') || e.message && e.message.includes('no encontrado')) {
                    try {
                        agency.barcode = Utils.generateAgencyBarcode(agency);
                        await DB.add('catalog_agencies', agency);
                    } catch (createError) {
                        console.warn(`Error creando agencia ${agency.id}:`, createError.message || createError);
                    }
                } else {
                    console.warn(`Error procesando agencia ${agency.id}:`, e.message || e);
                }
            }
        }));
        ...
        // Procesar vendedores en paralelo (mejor rendimiento)
        await Promise.all(sellers.map(async (seller) => {
            try {
                ...
                if (existing) {
                    ...
                    await DB.add('catalog_sellers', seller);
                }
            } catch (e) {
                // Si el error es 404, intentar crear el registro
                if (e.message && e.message.includes('404') || e.message && e.message.includes('no encontrado')) {
                    try {
                        ...
                        console.warn(`Error creando vendedor ${seller.id}:`, createError.message || createError);
                    }
                } else {
                    console.warn(`Error procesando vendedor ${seller.id}:`, e.message || e);
                }
            }
        }));
            ...
            for (const name of guideNames) {
                try {
                    ...
                    if (existing) {
                        ...
                        await DB.add('catalog_guides', guideData);
                    }
                } catch (e) {
                    // Si el error es 404, intentar crear el registro
                    if (e.message && e.message.includes('404') || e.message && e.message.includes('no encontrado')) {
                        try {
                            const guideId = `guide_${guideIdx - 1}`;
                            const guideData = {
                                id: guideId,
                                name: name,
                                agency_id: agencyId,
                                ...
                                active: true,
                                commission_rule: name === 'MARINA' ? 'guide_marina' : 'guide_default',
                                barcode: Utils.generateGuideBarcode({ id: guideId, name: name, agency_id: agencyId })
                            };
                            await DB.add('catalog_guides', guideData);
                        } catch (createError) {
                            console.warn(`Error creando guía ${guideId}:`, createError.message || createError);
                        }
                    } else {
                        console.warn(`Error procesando guía:`, e.message || e);
                    }
                }
            }
        }
        ...
        const branches = [
            { id: 'branch1', name: 'JOYERIA 1', address: '', active: true },
            { id: 'branch2', name: 'MALECON', address: '', active: true },
            { id: 'branch3', name: 'SAN SEBASTIAN', address: '', active: true },
        ...
        for (const rule of commissionRules) {
            try {
                ...
                // Usar formato SQL directamente (entity_type, entity_id, discount_pct, multiplier)
                const ruleData = {
                    id: rule.id,
                ...
                if (existing) {
                    ...
                    await DB.put('commission_rules', ruleData);
                } else {
                    await DB.put('commission_rules', ruleData);
                }
            } catch (e) {
    ...
    async loadArrivalRateRules() {
        try {
            // Verificar primero que la tabla existe antes de intentar cargar datos
            // Si la tabla no existe, simplemente omitir la precarga (no es crítica)
            ...
            if (existingRules.length >= 10) {
                console.log(`✅ Reglas de llegadas ya existen (${existingRules.length}), omitiendo precarga`);
                return;
            }
            
            // Si hay reglas existentes pero pocas, limpiarlas primero para recargar
            if (existingRules.length > 0 && existingRules.length < 10) {
                console.log('⚠️ Reglas incompletas detectadas, recargando...');
                for (const rule of existingRules) {
                    try {
                        await DB.delete('arrival_rate_rules', rule.id);
                    } catch (e) {
                        // Ignorar errores al eliminar
                        console.warn('Error eliminando regla:', e);
                    }
                }
            }
            ...
            if (tanitoursAgency) {
                rules.push(
                    { id: Utils.generateId(), agency_id: tanitoursAgency.id, branch_id: null, min_passengers: 11, max_passengers: 15, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1300, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TANITOURS: 11-15 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tanitoursAgency.id, branch_id: null, min_passengers: 16, max_passengers: 23, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1500, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TANITOURS: 16-23 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tanitoursAgency.id, branch_id: null, min_passengers: 24, max_passengers: 39, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 2500, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TANITOURS: 24-39 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' }
                );
            }
            ...
            if (discoveryAgency) {
                if (branch1) {
                    rules.push({ id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch1.id, min_passengers: 1, max_passengers: null, unit_type: 'city_tour', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 2000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: L Vallarta - City Tour', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' });
                }
                if (branch2) {
                    rules.push({ id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch2.id, min_passengers: 1, max_passengers: null, unit_type: 'city_tour', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 2000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: Malecón - City Tour', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' });
                }
                if (branch3) {
                    rules.push(
                        { id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch3.id, min_passengers: 1, max_passengers: null, unit_type: 'sprinter', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: San Sebastián - Sprinter', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                        { id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch3.id, min_passengers: 1, max_passengers: null, unit_type: 'van', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 600, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: San Sebastián - Van', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' }
                    );
                }
                if (branch4) {
                    rules.push(
                        { id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch4.id, min_passengers: 1, max_passengers: null, unit_type: 'sprinter', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: Sayulita - Sprinter', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                        { id: Utils.generateId(), agency_id: discoveryAgency.id, branch_id: branch4.id, min_passengers: 1, max_passengers: null, unit_type: 'van', rate_per_passenger: 0, fee_type: 'flat', flat_fee: 600, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'DISCOVERY: Sayulita - Van', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' }
                    );
                }
            }
            ...
            if (tbAgency) {
                rules.push(
                    { id: Utils.generateId(), agency_id: tbAgency.id, branch_id: null, min_passengers: 1, max_passengers: 6, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 300, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TB: 1-6 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tbAgency.id, branch_id: null, min_passengers: 7, max_passengers: 14, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 600, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TB: 7-14 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tbAgency.id, branch_id: null, min_passengers: 15, max_passengers: 18, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 800, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TB: 15-18 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tbAgency.id, branch_id: null, min_passengers: 20, max_passengers: 30, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TB: 20-30 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: tbAgency.id, branch_id: null, min_passengers: 30, max_passengers: 45, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1200, extra_per_passenger: 20, active_from: today, active_until: null, notes: 'TB: 30-45 PAX + $20 extra >45', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' }
                );
            }
            ...
            if (ttfAgency) {
                rules.push(
                    { id: Utils.generateId(), agency_id: ttfAgency.id, branch_id: null, min_passengers: 1, max_passengers: 6, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 300, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TTF: 1-6 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: ttfAgency.id, branch_id: null, min_passengers: 7, max_passengers: 14, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 600, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TTF: 7-14 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: ttfAgency.id, branch_id: null, min_passengers: 15, max_passengers: 18, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 800, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TTF: 15-18 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: ttfAgency.id, branch_id: null, min_passengers: 20, max_passengers: 30, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1000, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TTF: 20-30 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' },
                    { id: Utils.generateId(), agency_id: ttfAgency.id, branch_id: null, min_passengers: 30, max_passengers: 45, unit_type: null, rate_per_passenger: 0, fee_type: 'flat', flat_fee: 1200, extra_per_passenger: 0, active_from: today, active_until: null, notes: 'TTF: 30-45 PAX', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), sync_status: 'synced' }
                );
            }

            // Intentar agregar reglas solo si la tabla existe
            let addedCount = 0;
            for (const rule of rules) {
                try {
                    ...
                    // Si el error es que la tabla no existe, detener y no intentar más
                    if (e.message && (e.message.includes('no existe') || e.message.includes('does not exist') || e.message.includes('Tabla no existe'))) {
                        console.warn('⚠️ Tabla arrival_rate_rules no existe. Omitiendo precarga. Ejecuta la migración del backend.');
                        break; // No intentar más reglas
                    }
                    console.warn('⚠️ Error creando regla de llegada:', e.message || e);
                }
            }

            if (addedCount > 0) {
                console.log(`✅ ${addedCount} reglas de llegadas precargadas`);
            }
        } catch (e) {
            // Si el error es que la tabla no existe, solo loguear warning (no crítico)
            if (e.message && (e.message.includes('no existe') || e.message.includes('does not exist') || e.message.includes('Tabla no existe'))) {
                console.warn('⚠️ Tabla arrival_rate_rules no existe. Omitiendo precarga. Ejecuta la migración del backend desde /api/diagnose/migrate');
            } else {
                console.error('❌ Error loading arrival rate rules:', e);
            }
            // NO lanzar el error - permitir que el sistema continúe funcionando
        }
    },
    ...
    async loadWeeklyPayroll() {
        try {
            ...
            for (const cost of payrollCosts) {
                try {
                    // Normalizar datos antes de guardar (remover campos no soportados)
                    let normalizedCost;
                    if (typeof Costs !== 'undefined' && Costs.normalizeCostData) {
                        normalizedCost = Costs.normalizeCostData(cost);
                    } else {
                    ...
                    await DB.add('cost_entries', normalizedCost);
                    // Ya no se necesita SyncManager - el backend es centralizado y tiempo real
                } catch (e) {
    ...
    async loadInitialCosts() {
        try {
            ...
            for (const cost of initialCosts) {
                try {
                    // Normalizar datos usando la función de Costs si está disponible
                    let normalizedCost;
                    if (typeof Costs !== 'undefined' && Costs.normalizeCostData) {
                        normalizedCost = Costs.normalizeCostData(cost);
                    } else {
    ...
    async verifyAndFixBarcodes() {
        try {
            ...
            if (typeof API !== 'undefined' && !API.token) {
                console.log('verifyAndFixBarcodes: No hay token, omitiendo verificación hasta después del login');
                return;
            }
