...
const App = {
    ...
    async init() {
        ...
        // Ocultar pantallas de login y código de empresa
        const codeScreen = document.getElementById('company-code-screen');
        const loginScreen = document.getElementById('login-screen');
        if (codeScreen) codeScreen.style.display = 'none';
        if (loginScreen) loginScreen.style.display = 'none';
        ...
        try {
            ...
            // Initialize Branch Manager SOLO si el usuario ya está autenticado
            // Si no hay token, BranchManager se inicializará después del login exitoso
            if (typeof BranchManager !== 'undefined' && typeof API !== 'undefined' && API.token) {
                try {
            ...
            // Solo inicializar si hay token (se inicializará después del login si no hay token)
            if (typeof BranchSelector !== 'undefined' && BranchSelector.init && typeof API !== 'undefined' && API.token) {
            ...
            // Solo inicializar si hay token (se inicializará después del login si no hay token)
            if (typeof BackupManager !== 'undefined' && typeof API !== 'undefined' && API.token) {
            ...
            // Solo inicializar si hay token (se inicializará después del login si no hay token)
            if (typeof ExchangeRates !== 'undefined' && typeof API !== 'undefined' && API.token) {
...
// Debug code removed - DB ya no tiene .db (usando API ahora)
            ...
            // Cargar datos básicos del sistema solo si hay token (se cargará después del login si no hay token)
            ...
            // Solo si hay token (se ejecutará después del login)
            if (typeof Costs !== 'undefined' && Costs.generateAllWeeklyPayrolls && typeof API !== 'undefined' && API.token) {
