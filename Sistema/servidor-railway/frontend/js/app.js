...
// ELIMINADO529',message:'Employees module not defined',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    ...
    async loadArrivalRateRules() {
        try {
// ELIMINADO991',message:'loadArrivalRateRules START',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            const existingRules = await DB.getAll('arrival_rate_rules') || [];
// ELIMINADO995',message:'existingRules count',data:{count:existingRules.length,rules:existingRules.map(r=>({id:r.id,agency:r.agency_id,min:r.min_passengers,max:r.max_passengers}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
...
// ELIMINADO998',message:'Skipping precarga - enough rules',data:{count:existingRules.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                return;
            }
...
// ELIMINADO1005',message:'Clearing incomplete rules',data:{count:existingRules.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
...
// ELIMINADO1084',message:'About to save rules',data:{count:rules.length,rules:rules.map(r=>({agency:r.agency_id,min:r.min_passengers,max:r.max_passengers,fee_type:r.fee_type,flat_fee:r.flat_fee}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
...
// ELIMINADO1093',message:'Precarga completada',data:{count:rules.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        } catch (e) {
