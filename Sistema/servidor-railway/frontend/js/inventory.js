...
const Inventory = {
    ...
    async init() {
        // ELIMINADOinventory.js:18',message:'Inventory.init called',data:{initialized:this.initialized,hasPermissionManager:typeof PermissionManager!=='undefined',currentUser:typeof UserManager!=='undefined'?UserManager.currentUser:null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'F'})}).catch(()=>{});
        ...
        if (!hasPermission) {
            ...
            // ELIMINADOinventory.js:28',message:'Permission denied, returning early',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
            return;
        }
        ...
        // Escuchar eventos WebSocket para actualización en tiempo real
        this.setupWebSocketListeners();
        
        // Escuchar cambios en el filtro de sucursal (para admin)
        window.addEventListener('branch-filter-changed', () => {
            if (this.initialized) {
                this.loadInventory();
            }
        });
    ...
    // ELIMINADOinventory.js:45',message:'Checking add button permission in setupUI',data:{hasPermissionManager:typeof PermissionManager!=='undefined',hasPermission:hasPermission,userRole:typeof UserManager!=='undefined'?UserManager.currentUser?.role:null},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                            ...
                            <div style="display: flex; gap: 4px; margin-left: auto;">
                                <button class="btn-secondary btn-sm inventory-view-btn ${this.currentView === 'grid' ? 'active' : ''}" data-view="grid" title="Vista Grid">
                                    <i class="fas fa-th"></i>
                                </button>
                                <button class="btn-secondary btn-sm inventory-view-btn ${this.currentView === 'gallery' ? 'active' : ''}" data-view="gallery" title="Vista Galería">
                                    <i class="fas fa-th-large"></i>
                                </button>
                                <button class="btn-secondary btn-sm inventory-view-btn ${this.currentView === 'list' ? 'active' : ''}" data-view="list" title="Vista Lista">
    ...
    async setupEventListeners() {
        ...
        document.querySelectorAll('.inventory-view-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                document.querySelectorAll('.inventory-view-btn').forEach(b => b.classList.remove('active'));
                e.currentTarget.classList.add('active');
                localStorage.setItem('inventory_view', view);
                this.loadInventory();
            });
        });
        ...
        if (savedView && ['grid', 'gallery', 'list'].includes(savedView)) {
        ...
        if (branchFilter) {
            // Verificar que los catálogos estén cargados antes de usarlos
            let branches = await DB.getAll('catalog_branches') || [];
            
            // Si no hay datos, esperar un poco y reintentar (máximo 5 intentos)
            let attempts = 0;
            while (branches.length === 0 && attempts < 5) {
    ...
    async loadInventory() {
        ...
        try {
            ...
            // Obtener TODOS los items sin filtro primero para verificar que realmente existen
            // Usar una consulta directa sin cache
            let allItemsRaw = [];
            try {
                // Obtener directamente desde la API (DB.getAll ahora usa la API)
            ...
            console.log(`📦 Items obtenidos de la API: ${allItemsRaw.length}`);
            ...
            if (ghostItems.length > 0) {
                ...
                // Intentar eliminar items fantasma usando la API
                try {
                        for (const ghostId of ghostItems) {
                            try {
                            // Intentar eliminar usando la API
                            if (typeof API !== 'undefined' && API.deleteInventoryItem) {
                                await API.deleteInventoryItem(ghostId);
                            } else {
                                // Fallback: intentar eliminar usando DB.delete
            ...
            if (!viewAllBranches && currentBranchId) {
                ...
                items = items.filter(item => {
                    ...
                    if (!item.branch_id) {
                        console.warn(`⚠️ Item sin branch_id excluido: ${item.id} (${item.sku || item.name})`);
                        return false;
                    }
    ...
    // ELIMINADOinventory.js:260',message:'No items after filter, checking all items',data:{allItemsCount:allItems.length,itemsWithoutBranch:allItems.filter(i=>!i.branch_id).length,branchIds:allItems.filter(i=>i.branch_id).map(i=>i.branch_id),currentBranchId:currentBranchId},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
            ...
            // ELIMINADOinventory.js:330',message:'About to display inventory',data:{itemsCount:items.length,containerExists:!!document.getElementById('inventory-list')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            
            this.displayInventory(items);
            
            // ELIMINADOinventory.js:333',message:'Inventory displayed',data:{itemsCount:items.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        } catch (e) {
            // ELIMINADOinventory.js:336',message:'Error loading inventory',data:{error:e.message,errorStack:e.stack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
    ...
    async displayInventory(items) {
        // ELIMINADOinventory.js:398',message:'displayInventory called',data:{itemsCount:items.length,containerExists:!!document.getElementById('inventory-list')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        
        const container = document.getElementById('inventory-list');
        if (!container) {
            // ELIMINADOinventory.js:404',message:'Container not found in displayInventory',data:{itemsCount:items.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            return;
        }
        ...
        if (this.currentView === 'gallery') {
            container.innerHTML = this.renderGalleryView(itemsWithPhotos);
        } else if (this.currentView === 'list') {
            container.innerHTML = this.renderListView(itemsWithPhotos);
        } else {
            container.innerHTML = this.renderGridView(itemsWithPhotos);
        }
    },
            ...
            <div class="inventory-card ${isSelected ? 'inventory-card-selected' : ''}" data-item-id="${item.id}">
                ...
                <div class="inventory-card-info">
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                        <div style="flex: 1;">
                            <h4 style="margin: 0;">${item.name || item.sku}</h4>
                            ${item.branch_name ? `<div style="margin-top: 4px;"><span class="badge badge-info" style="font-size: 10px; padding: 2px 6px;">🏢 ${item.branch_name}</span></div>` : ''}
                        </div>
    ...
    renderGalleryView(items) {
        return `<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: var(--spacing-md);">
            ${items.map(item => {
                const isSelected = this.selectedItems.has(item.id);
                const stockStatus = this.getStockStatus(item);
                const stockBadgeClass = this.getStockBadgeClass(stockStatus);
                return `
                <div class="inventory-gallery-card ${isSelected ? 'inventory-card-selected' : ''}" data-item-id="${item.id}" style="position: relative; cursor: pointer;" onclick="window.Inventory.showItemDetails('${item.id}')">
                    <input type="checkbox" class="inventory-checkbox" 
                           ${isSelected ? 'checked' : ''} 
                           onclick="event.stopPropagation(); window.Inventory.toggleItemSelection('${item.id}', this.checked)"
                           style="position: absolute; top: 8px; left: 8px; z-index: 10;">
                    ${item.photo ? 
                        `<img src="${item.photo}" alt="${item.name}" style="width: 100%; height: 200px; object-fit: cover; border-radius: var(--radius-sm);">` : 
                        `<div style="width: 100%; height: 200px; display: flex; align-items: center; justify-content: center; background: var(--color-bg-secondary); border-radius: var(--radius-sm);"><i class="fas fa-gem" style="font-size: 48px; opacity: 0.3;"></i></div>`
                    }
                    <div style="padding: var(--spacing-sm);">
                        <h4 style="margin: 0 0 4px 0; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${item.name || item.sku}</h4>
                        <p style="font-size: 11px; color: var(--color-text-secondary); margin: 0 0 4px 0;">${item.sku}</p>
                        <p style="font-size: 13px; font-weight: 600; color: var(--color-primary); margin: 4px 0;">${Utils.formatCurrency(item.cost || 0)}</p>
                        <span class="stock-badge ${stockBadgeClass}" style="font-size: 10px;">${this.getStockStatusText(stockStatus)}</span>
                    </div>
                </div>
                `;
            }).join('')}
    ...
    renderListView(items) {
        return `<table class="data-table" style="width: 100%;">
            <thead>
                <tr>
                    <th style="width: 40px;"><input type="checkbox" onchange="window.Inventory.toggleSelectAll()"></th>
                    <th>Foto</th>
                    <th>SKU</th>
                    <th>Nombre</th>
                    <th>Metal</th>
            ...
            <tbody>
                ${items.map(item => {
                    ...
                    const stockBadgeClass = this.getStockBadgeClass(stockStatus);
                    return `
                    <tr class="${isSelected ? 'row-selected' : ''}" data-item-id="${item.id}">
                        <td><input type="checkbox" ${isSelected ? 'checked' : ''} onchange="window.Inventory.toggleItemSelection('${item.id}', this.checked)"></td>
                        <td>${item.photo ? `<img src="${item.photo}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;">` : '<i class="fas fa-gem" style="opacity: 0.3;"></i>'}</td>
                        <td>${item.sku}</td>
                        <td>${item.name}</td>
                        <td>${item.metal || 'N/A'}</td>
                        <td>${item.stone || 'N/A'}</td>
                        <td>${item.weight_g || 0}g</td>
                        <td>${Utils.formatCurrency(item.cost || 0)}</td>
                        <td><span class="stock-badge ${stockBadgeClass}">${this.getStockStatusText(stockStatus)}</span></td>
                        <td><span class="status-badge status-${item.status}">${item.status}</span></td>
                        <td>
                            <button class="btn-secondary btn-xs" onclick="window.Inventory.showItemDetails('${item.id}')">Ver</button>
                            ${typeof PermissionManager !== 'undefined' && PermissionManager.hasPermission('inventory.update') ? 
                                `<button class="btn-secondary btn-xs" onclick="window.Inventory.showAddForm('${item.id}')">Editar</button>` : ''}
                        </td>
                    </tr>
                    `;
                }).join('')}
    ...
    async deleteItem(itemId) {
        try {
            ...
            // La eliminación se sincroniza automáticamente con el backend
    ...
    async saveItem(itemId) {
        ...
        const metalInput = document.getElementById('inv-metal');
        const sizeInput = document.getElementById('inv-size');
        const weightInput = document.getElementById('inv-weight');
        const costInput = document.getElementById('inv-cost');
        const locationInput = document.getElementById('inv-location');
        const statusInput = document.getElementById('inv-status');

        let isValid = true;

        // Validar nombre
        if (typeof Validators !== 'undefined') {
            if (!Validators.string(nameInput?.value, { required: true, minLength: 3, maxLength: 200 }).valid) {
                isValid = false;
                if (nameInput) {
                    nameInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El nombre es requerido y debe tener entre 3 y 200 caracteres';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    nameInput.parentNode.insertBefore(error, nameInput.nextSibling);
                }
            }

            // Validar metal
            if (!Validators.string(metalInput?.value, { required: true }).valid) {
                isValid = false;
                if (metalInput) {
                    metalInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El metal es requerido';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    metalInput.parentNode.insertBefore(error, metalInput.nextSibling);
                }
            }

            // Validar tamaño
            if (!Validators.string(sizeInput?.value, { required: true }).valid) {
                isValid = false;
                if (sizeInput) {
                    sizeInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El tamaño es requerido';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    sizeInput.parentNode.insertBefore(error, sizeInput.nextSibling);
                }
            }

            // Validar peso
            const weightResult = Validators.number(weightInput?.value, { required: true, min: 0 });
            if (!weightResult.valid) {
                isValid = false;
                if (weightInput) {
                    weightInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = weightResult.error || 'El peso debe ser un número mayor o igual a 0';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    weightInput.parentNode.insertBefore(error, weightInput.nextSibling);
                }
            }

            // Validar costo
            const costResult = Validators.number(costInput?.value, { required: true, min: 0 });
            if (!costResult.valid) {
                isValid = false;
                if (costInput) {
                    costInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = costResult.error || 'El costo debe ser un número mayor o igual a 0';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    costInput.parentNode.insertBefore(error, costInput.nextSibling);
                }
            }

            // Validar ubicación
            if (!Validators.string(locationInput?.value, { required: true }).valid) {
                isValid = false;
                if (locationInput) {
                    locationInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'La ubicación es requerida';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    locationInput.parentNode.insertBefore(error, locationInput.nextSibling);
                }
            }

            // Validar estado
            if (!Validators.string(statusInput?.value, { required: true }).valid) {
                isValid = false;
                if (statusInput) {
                    statusInput.classList.add('field-invalid');
                    const error = document.createElement('div');
                    error.className = 'field-error';
                    error.textContent = 'El estado es requerido';
                    error.style.cssText = 'color: #d32f2f; font-size: 12px; margin-top: 4px;';
                    statusInput.parentNode.insertBefore(error, statusInput.nextSibling);
                }
            }
        } else {
            // Fallback a validación HTML5 básica
        ...
        // ELIMINADOinventory.js:1370',message:'Branch ID determined',data:{branchId:branchId,fromBranchManager:typeof BranchManager!=='undefined',fromLocalStorage:localStorage.getItem('current_branch_id')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        ...
        const isEdit = !!itemId;
        const existingItem = isEdit ? await DB.get('inventory_items', itemId) : null;
        
        // Si es edición y el producto no existe, mostrar error
        if (isEdit && !existingItem) {
            Utils.showNotification('El producto que intentas editar no existe', 'error');
            return;
        }
        
        // Preparar datos para el backend (solo campos que existen en la tabla)
        const item = {
            ...
            description: document.getElementById('inv-notes')?.value?.trim() || null,
            metal: document.getElementById('inv-metal').value || null,
            stone: document.getElementById('inv-stone')?.value?.trim() || null,
            size: document.getElementById('inv-size').value?.trim() || null,
            weight: parseFloat(document.getElementById('inv-weight').value) || null, // Backend espera 'weight'
            dimensions: document.getElementById('inv-measures')?.value?.trim() || null, // Backend espera 'dimensions'
            cost: parseFloat(document.getElementById('inv-cost').value) || 0,
            price: 0, // Precio de venta se asigna manualmente en el POS
            stock: stockActual, // Backend espera 'stock' (usar stock_actual como valor)
            branch_id: branchId,
            location: document.getElementById('inv-location').value?.trim() || null,
            status: document.getElementById('inv-status').value || 'disponible'
        };
        
        // Campos adicionales para uso interno del frontend (no se envían al backend)
        const frontendData = {
        ...
        try {
            // Usar DB.add para crear nuevo, DB.put para actualizar
            let savedItem;
            if (isEdit) {
                // Actualizar producto existente
                savedItem = await DB.put('inventory_items', item);
            } else {
                // Crear nuevo producto (sin ID)
                savedItem = await DB.add('inventory_items', item);
            }
            ...
            // Obtener el ID del producto guardado (puede venir del servidor o ser el que enviamos)
            const finalItemId = savedItem.id || (isEdit ? itemId : null);
            
            if (!finalItemId) {
                throw new Error('No se recibió ID del producto guardado');
            }
            
            // Registrar cambio de stock si es edición y cambió
            if (isEdit && existingItem) {
                const existingStock = existingItem.stock || existingItem.stock_actual || 0;
                if (existingStock !== stockActual) {
                    try {
                        const logId = Utils.generateId();
                        await DB.add('inventory_logs', {
                            id: logId,
                            item_id: finalItemId,
                            action: stockActual > existingStock ? 'entrada' : 'salida',
                            quantity: Math.abs(stockActual - existingStock),
                            stock_before: existingStock,
                            stock_after: stockActual,
                            reason: 'edicion',
                            notes: `Stock modificado durante edición de ${existingStock} a ${stockActual}`,
                            created_at: new Date().toISOString()
                        });
                    } catch (logError) {
                        console.warn('Error guardando log de stock:', logError);
                    }
                }
            }

            // Guardar certificado si existe
            if (frontendData.certificate_type && frontendData.certificate_number) {
                try {
                    const existingCert = isEdit ? await DB.query('inventory_certificates', 'item_id', finalItemId).then(certs => certs?.[0] || null).catch(() => null) : null;
                    const certificate = {
                        id: existingCert?.id || Utils.generateId(),
                        item_id: finalItemId,
                        certificate_type: frontendData.certificate_type,
                        certificate_number: frontendData.certificate_number,
                    ...
                    console.warn('Error guardando certificado:', certError);
                }
            }

            // Guardar historial de precio si cambió (solo en edición)
            if (isEdit && existingItem) {
                ...
                if (oldCost !== item.cost) {
                    try {
                        ...
                        console.warn('Error guardando historial de precio:', priceError);
                    }
                }
            }
            ...
            if (photoInput && photoInput.files.length > 0) {
                for (const file of photoInput.files) {
                    try {
                        ...
                        console.warn('Error guardando foto:', photoError);
                    }
                }
            }
            ...
            try {
                ...
                await DB.add('inventory_logs', {
                    id: logId,
                    item_id: finalItemId,
                    action: isEdit ? 'actualizada' : 'alta',
                    quantity: 1,
                    notes: isEdit ? 'Pieza actualizada' : 'Nueva pieza',
                    created_at: new Date().toISOString()
                });
            } catch (logError) {
                console.warn('Error guardando log de inventario:', logError);
            }
            
            Utils.showNotification(isEdit ? 'Producto actualizado' : 'Producto creado', 'success');
            UI.closeModal();
            await this.loadInventory();
        } catch (error) {
            console.error('Error guardando producto:', error);
            const errorMessage = error.message || error.error || 'Error desconocido';
            Utils.showNotification('Error al guardar producto: ' + errorMessage, 'error');
            throw error;
        }
        ...
        // ELIMINADOinventory.js:1520',message:'About to reload inventory after save',data:{itemId:item.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
        ...
        // ELIMINADOinventory.js:1525',message:'Inventory reloaded after save',data:{itemId:item.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
    },
