...
// Manejar preflight OPTIONS (necesario incluso para mismo dominio en algunos casos)
app.options('*', (req, res) => {
    const origin = req.headers.origin;
    
    // Permitir mismo origen o cualquier origen (ya que será todo desde Railway)
    res.setHeader('Access-Control-Allow-Origin', origin || '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS, PATCH');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, Accept, Origin, X-CSRF-Token, csrf-token');
    res.setHeader('Access-Control-Allow-Credentials', 'true');
    res.setHeader('Access-Control-Max-Age', '86400');
    res.sendStatus(200);
});

// Middleware CORS simplificado (todo desde mismo dominio ahora)
app.use((req, res, next) => {
    const origin = req.headers.origin;
    
    // Agregar headers CORS básicos (necesarios incluso para mismo dominio en algunos casos)
    if (origin) {
        res.setHeader('Access-Control-Allow-Origin', origin);
    }
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS, PATCH');
...
// Middleware CORS usando el paquete cors (simplificado)
app.use(cors({
    origin: true, // Permitir cualquier origen (simplificado para monolito)
    credentials: true,
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'PATCH'],
    allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'Origin', 'X-CSRF-Token', 'csrf-token'],
...
const io = new Server(server, {
    cors: {
        origin: true, // Permitir cualquier origen (simplificado)
        methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'PATCH'],
        allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'Origin', 'X-CSRF-Token', 'csrf-token'],
        credentials: true
    },
...
// Aplicar rate limiting general (ANTES de otras rutas, pero después de CORS)
// Las rutas específicas pueden tener sus propios rate limiters
app.use((req, res, next) => {
    // Aplicar rate limiting solo si no está en las rutas excluidas
    const skipPaths = [
        '/health',
        '/api/info',
        '/api/csrf-token',
    ...
    if (skipPaths.some(path => req.path.startsWith(path))) {
        return next(); // Saltar rate limiting
    }
