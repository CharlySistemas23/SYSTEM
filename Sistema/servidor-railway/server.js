...
const isOriginAllowed = (origin) => {
    ...
    // Permitir cualquier dominio .vercel.app, .netlify.app, .railway.app, etc.
    ...
    // Permitir el mismo dominio del servidor (cuando frontend está servido desde Railway)
    // Railway puede usar diferentes variables de entorno para el dominio público
    const serverUrl = process.env.RAILWAY_PUBLIC_DOMAIN || 
                     process.env.RAILWAY_STATIC_URL ||
                     process.env.RAILWAY_PUBLIC_URL ||
                     'https://system-production-9e21.up.railway.app';
    
    // Comparar sin protocolo para mayor flexibilidad
    const originHost = origin.replace(/^https?:\/\//, '');
    const serverHost = serverUrl.replace(/^https?:\/\//, '');
    
    if (originHost === serverHost || originHost.startsWith(serverHost) || serverHost.startsWith(originHost)) {
        return true;
    }
    ...
    try {
        ...
        if (!tablesExist) {
            ...
            try {
                ...
                // Ejecutar seed para crear datos iniciales (sucursal, empleado admin, usuario admin)
                console.log('');
                ...
                try {
                    ...
                    // No detener el servidor si falla el seed, pero registrar el error
                    console.warn('⚠️  Continuando sin datos iniciales. Puedes ejecutar manualmente: npm run seed');
                }
            } catch (migrateError) {
                console.error('');
            ...
            // Verificar si hay datos iniciales (sucursal, usuario)
            console.log('🌱 Verificando datos iniciales...');
            try {
                const { seedDatabase } = await import('./database/seed.js');
                await seedDatabase();
