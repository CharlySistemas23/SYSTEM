...
const Repairs = {
    ...
    async init() {
        try {
            ...
            console.error('❌ Error inicializando módulo Repairs:', error);
            ...
            if (content) {
                content.innerHTML = `
                    <div style="padding: var(--spacing-lg); text-align: center;">
                        <h3 style="color: var(--color-danger);">Error al cargar módulo Repairs</h3>
    ...
    setupUI() {
        const content = document.getElementById('module-content');
        if (!content) {
            console.error('❌ module-content no encontrado en setupUI');
            return;
        }
        ...
        content.innerHTML = `
            <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light); margin-bottom: var(--spacing-md);">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-sm);">
                    ...
                    <div class="form-group" id="repair-branch-filter-container" style="margin-bottom: 0; display: none;">
                        <label style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; margin-bottom: var(--spacing-xs);">Sucursal</label>
                        <select id="repair-branch-filter" class="form-select">
    ...
    async loadRepairs() {
        try {
            ...
            const branchFilterEl = document.getElementById('repair-branch-filter');
            ...
            // PASO 1: Subir reparaciones locales que NO están en el servidor
            try {
                if (typeof API !== 'undefined' && API.baseURL && API.token && API.createRepair && API.updateRepair) {
                    console.log('📤 [Paso 1 Repairs] Buscando reparaciones locales que no están en el servidor...');
                    
                    const allLocalRepairs = await DB.getAll('repairs') || [];
                    const unsyncedRepairs = allLocalRepairs.filter(r => {
                        if (!r || !r.id) return false;
                        const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(r.id));
                        return !isUUID || !r.server_id;
                    });
                    
                    const repairsToSync = unsyncedRepairs.filter(r => {
                        if (!filterBranchId) return true;
                        return r.branch_id === filterBranchId;
                    });
                    
                    console.log(`📊 [Paso 1 Repairs] Encontradas ${repairsToSync.length} reparaciones locales sin sincronizar`);
                    
                    if (repairsToSync.length > 0) {
                        const repairsByKey = new Map();
                        for (const localRepair of repairsToSync) {
                            const key = `${localRepair.repair_number || localRepair.folio || localRepair.id}_${localRepair.branch_id || 'no-branch'}`;
                            if (!repairsByKey.has(key)) {
                                repairsByKey.set(key, localRepair);
                            } else {
                                const existing = repairsByKey.get(key);
                                const existingUpdated = existing.updated_at ? new Date(existing.updated_at) : new Date(0);
                                const currentUpdated = localRepair.updated_at ? new Date(localRepair.updated_at) : new Date(0);
                        ...
                        let uploadedCount = 0;
                        for (const [key, localRepair] of repairsByKey) {
                            try {
                                console.log(`📤 [Paso 1 Repairs] Subiendo reparación local al servidor: ${localRepair.id}`);
                                
                                let serverRepair = null;
                                if (localRepair.repair_number || localRepair.folio) {
                                    try {
                                        const serverRepairs = await API.getRepairs({ 
                                            branch_id: filterBranchId,
                                            repair_number: localRepair.repair_number || localRepair.folio
                                        });
                                        if (serverRepairs && serverRepairs.length > 0) {
                                            serverRepair = serverRepairs[0];
                                        }
                                    } catch (e) {}
                                }
                                
                                if (serverRepair && serverRepair.id) {
                                    const updatedRepair = await API.updateRepair(serverRepair.id, localRepair);
                                    if (updatedRepair && updatedRepair.id) {
                                        const allLocalRepairs = await DB.getAll('repairs') || [];
                                        const repairsToUpdate = allLocalRepairs.filter(r => {
                                            const rKey = `${r.repair_number || r.folio || r.id}_${r.branch_id || 'no-branch'}`;
                                            return rKey === key;
                                        });
                                        
                                        for (const repairToUpdate of repairsToUpdate) {
                                            repairToUpdate.server_id = updatedRepair.id;
                                            repairToUpdate.id = updatedRepair.id;
                                            repairToUpdate.sync_status = 'synced';
                                            await DB.put('repairs', repairToUpdate, { autoBranchId: false });
                                        }
                                        uploadedCount++;
                                    }
                                } else {
                                    const createdRepair = await API.createRepair(localRepair);
                                    if (createdRepair && createdRepair.id) {
                                        const allLocalRepairs = await DB.getAll('repairs') || [];
                                        const repairsToUpdate = allLocalRepairs.filter(r => {
                                            const rKey = `${r.repair_number || r.folio || r.id}_${r.branch_id || 'no-branch'}`;
                                            return rKey === key;
                                        ...
                                        for (const repairToUpdate of repairsToUpdate) {
                                            repairToUpdate.server_id = createdRepair.id;
                                            repairToUpdate.id = createdRepair.id;
                                            repairToUpdate.sync_status = 'synced';
                                            await DB.put('repairs', repairToUpdate, { autoBranchId: false });
                                        }
                                        uploadedCount++;
                                ...
                                console.error(`❌ [Paso 1 Repairs] Error subiendo reparación ${localRepair.id}:`, uploadError);
                            }
                        }
                        
                        console.log(`✅ [Paso 1 Repairs] Sincronización local→servidor completada: ${uploadedCount} reparaciones subidas`);
                    }
                }
            } catch (error) {
                console.error('❌ [Paso 1 Repairs] Error sincronizando reparaciones locales al servidor:', error);
            }

            // CACHE-FIRST: Cargar desde IndexedDB primero
            const fromIdb = await DB.getAll('repairs', null, null, { filterByBranch: false }) || [];
            const norm = id => String(id || '').trim();
            let cachedFiltered = fromIdb;
            if (filterBranchId) {
                const fb = norm(filterBranchId);
                cachedFiltered = fromIdb.filter(r => r.branch_id && norm(r.branch_id) === fb);
            } else if (!viewAllBranches && currentBranchId) {
                const cb = norm(currentBranchId);
                cachedFiltered = fromIdb.filter(r => r.branch_id && norm(r.branch_id) === cb);
            }
            const hasCache = cachedFiltered.length > 0;
            const hasApi = typeof API !== 'undefined' && API.baseURL && API.token && API.getRepairs;

            let repairs = [];

            if (hasCache) {
                repairs = cachedFiltered;
                console.log(`⚡ [Cache-First] ${repairs.length} reparaciones desde IndexedDB`);
                if (hasApi) {
                    Promise.resolve().then(async () => {
                        try {
                            const fresh = await API.getRepairs({ branch_id: filterBranchId });
                            for (const r of fresh) {
                                try {
                                    await DB.put('repairs', { ...r, server_id: r.id, sync_status: 'synced' }, { autoBranchId: false });
                                } catch (e) {}
                            }
                            const beforeIds = new Set(repairs.map(r => r.id));
                            const afterIds = new Set(fresh.map(r => r.id));
                            if (beforeIds.size !== afterIds.size || [...beforeIds].some(id => !afterIds.has(id))) {
                                await this.loadRepairs();
                            }
                        } catch (e) { console.warn('Sync repairs background:', e); }
                    }).catch(e => console.warn('Sync repairs background:', e));
                }
            } else if (hasApi) {
                const content = document.getElementById('repairs-content') || document.getElementById('module-content');
                if (content) content.innerHTML = '<div style="padding: var(--spacing-lg); text-align: center;"><i class="fas fa-spinner fa-spin"></i> Cargando reparaciones...</div>';
                try {
                    console.log('📥 [Paso 2 Repairs] Sincronizando reparaciones desde el servidor...');
                    repairs = await API.getRepairs({ branch_id: filterBranchId });
                    console.log(`📥 [Paso 2 Repairs] ${repairs.length} reparaciones recibidas del servidor`);
                    
                    // Guardar/actualizar cada reparación en IndexedDB local
                    let savedCount = 0;
                    let updatedCount = 0;
                    for (const serverRepair of repairs) {
                        try {
                            const key = `${serverRepair.repair_number || serverRepair.folio || serverRepair.id}_${serverRepair.branch_id || 'no-branch'}`;
                            const existingLocalRepairs = await DB.getAll('repairs') || [];
                            const existingRepair = existingLocalRepairs.find(r => {
                                const rKey = `${r.repair_number || r.folio || r.id}_${r.branch_id || 'no-branch'}`;
                                return rKey === key;
                            });
                            ...
                            const localRepair = {
                                ...serverRepair,
                                server_id: serverRepair.id,
                                sync_status: 'synced'
                            };
                            
                            await DB.put('repairs', localRepair, { autoBranchId: false });
                            ...
                            console.warn(`⚠️ [Paso 2 Repairs] Error guardando reparación ${serverRepair.id}:`, error);
                        }
                    }
                    
                    console.log(`✅ [Paso 2 Repairs] Sincronización servidor→local completada: ${savedCount} nuevas, ${updatedCount} actualizadas`);
                    
                    // CRÍTICO: Aplicar filtro estricto DESPUÉS de recibir de API
                    if (filterBranchId) {
                        const beforeStrictFilter = repairs.length;
                        repairs = repairs.filter(r => {
                            if (!r.branch_id) return false;
                            return String(r.branch_id) === String(filterBranchId);
                        });
                        console.log(`📍 Repairs: Filtrado estricto API: ${beforeStrictFilter} → ${repairs.length} (sucursal: ${filterBranchId})`);
                    }
                } catch (apiError) {
            ...
            for (const repair of repairs) {
                const key = `${repair.repair_number || repair.folio || repair.id}_${repair.branch_id || 'no-branch'}`;
                if (!repairsByKey.has(key)) {
                    repairsByKey.set(key, repair);
                } else {
                    const existing = repairsByKey.get(key);
                    if (repair.server_id && !existing.server_id) {
                        repairsByKey.set(key, repair);
                    } else if (existing.server_id && !repair.server_id) {
                        // Mantener el existente
                    } else {
                        const existingUpdated = existing.updated_at ? new Date(existing.updated_at) : new Date(0);
                        const currentUpdated = repair.updated_at ? new Date(repair.updated_at) : new Date(0);
                        if (currentUpdated > existingUpdated) {
                            repairsByKey.set(key, repair);
                        }
                    }
            ...
            repairs = Array.from(repairsByKey.values());
            console.log(`🔍 [Paso 3 Repairs] Deduplicación: ${repairs.length} reparaciones únicas`);
            
            // Filtrado adicional por sucursal si hay filtro específico
            if (filterBranchId) {
                const normalizedBranchId = String(filterBranchId);
                repairs = repairs.filter(r => {
                    if (!r.branch_id) return false; // Excluir reparaciones sin sucursal
            ...
            // SIEMPRE renderizar, incluso si no hay reparaciones
            try {
                await this.displayRepairs(repairs);
            } catch (renderError) {
                console.error('❌ Error renderizando reparaciones:', renderError);
                const repairsList = document.getElementById('repairs-list');
                if (repairsList) {
                    repairsList.innerHTML = `<div style="padding: var(--spacing-lg); text-align: center; color: var(--color-danger);">Error al renderizar: ${renderError.message}</div>`;
                }
            }
        } catch (e) {
            console.error('Error loading repairs:', e);
            const errorMsg = e.message || 'Error desconocido al cargar reparaciones';
            Utils.showNotification(`Error al cargar reparaciones: ${errorMsg}`, 'error');
            ...
            if (repairsList) {
                repairsList.innerHTML = `
                    <div style="padding: var(--spacing-lg); text-align: center; color: var(--color-danger);">
                        <h3>Error al cargar reparaciones</h3>
                        <p>${errorMsg}</p>
                        <button class="btn-primary" onclick="window.Repairs.loadRepairs()" style="margin-top: var(--spacing-md);">
    ...
    async displayRepairs(repairs) {
        ...
        if (!container) {
            ...
            if (content) {
                ...
                const retryContainer = document.getElementById('repairs-list');
                if (!retryContainer) {
                    console.error('❌ No se pudo crear repairs-list después de reconfigurar');
                    if (content) {
                        content.innerHTML = '<div style="padding: var(--spacing-lg); text-align: center; color: var(--color-danger);">Error: No se pudo inicializar el módulo de reparaciones. Por favor, recarga la página.</div>';
                    }
                    return;
                }
                // Reintentar con el contenedor recién creado
                return this.displayRepairs(repairs);
            }
            return;
        }
    ...
    async saveRepair(repairId) {
        ...
        const repair = {
            ...
            estimated_cost: parseFloat(document.getElementById('repair-cost').value) || 0,
            estimated_delivery_date: document.getElementById('repair-delivery-date')?.value || null,
        ...
        if (typeof API !== 'undefined' && API.baseURL && API.token) {
            try {
                ...
                const payload = {
                    folio: repair.folio,
                    branch_id: (repair.branch_id && isUUID(repair.branch_id)) ? repair.branch_id : undefined,
                    customer_id: (repair.customer_id && isUUID(repair.customer_id)) ? repair.customer_id : undefined,
                    description: repair.description,
                    estimated_cost: repair.estimated_cost || repair.cost || 0,
                    estimated_delivery_date: repair.estimated_delivery_date || null,
                    notes: repair.notes || '',
                ...
                if (repairId) {
                    ...
                    console.log('📤 Actualizando reparación en servidor...');
                    const updated = await API.updateRepair(repairId, payload);
                    if (updated) {
                        repair = { ...repair, ...updated };
                        savedWithAPI = true;
                        console.log('✅ Reparación actualizada en servidor');
                    }
                } else {
                    // Crear nueva reparación
                    console.log('📤 Creando reparación en servidor...');
                    const created = await API.createRepair(payload);
                    if (created && created.id) {
                        // Si el servidor generó un ID diferente, actualizar el local
                        if (created.id !== repair.id) {
                            console.log(`🔄 Actualizando ID local: ${repair.id} → ${created.id}`);
                            await DB.delete('repairs', repair.id);
                            repair.id = created.id;
                        }
                        repair = { ...repair, ...created };
                        savedWithAPI = true;
                        console.log('✅ Reparación creada en servidor');
                    }
                }
            } catch (apiError) {
                console.warn('⚠️ Error guardando reparación en servidor (continuando con modo local):', apiError);
        ...
        console.log('✅ Reparación guardada en IndexedDB');
        ...
        // Agregar a cola de sincronización solo si NO se guardó con API (o como respaldo)
        if (typeof SyncManager !== 'undefined') {
            try {
                if (!savedWithAPI) {
                    // Si no se guardó con API, agregar a cola para sincronización posterior
                    await SyncManager.addToQueue('repair', repair.id);
                } else {
                    // Si ya se guardó con API, aún así agregar a cola para otros clientes
                    await SyncManager.addToQueue('repair', repair.id);
                }
            } catch (syncError) {
                console.error('Error agregando repair a cola:', syncError);
            }
        }
    ...
    async setupBranchFilter() {
        const branchFilterContainer = document.getElementById('repair-branch-filter-container');
        const branchFilter = document.getElementById('repair-branch-filter');
        ...
        if (!isMasterAdmin) {
            ...
            newBranchFilter.addEventListener('change', () => this.loadRepairs());
        }
    ...
    setupSocketListeners() {
        ...
        window.addEventListener('repair-updated-all-branches', async (e) => {
                ...
                if (this.initialized) {
                    ...
                    // Recargar reparaciones después de un pequeño delay
                    setTimeout(async () => {
                        await this.loadRepairs();
                    }, 300);
                }
        ...
        window.addEventListener('repair-updated', async (e) => {
            ...
            if (this.initialized) {
                ...
                const currentBranchId = typeof BranchManager !== 'undefined' ? BranchManager.getCurrentBranchId() : null;
                // Solo actualizar si la reparación es de la sucursal actual
                if (repair && (!currentBranchId || repair.branch_id === currentBranchId)) {
                    console.log(`🔧 Repairs: Reparación actualizada localmente`);
                    setTimeout(async () => {
                        await this.loadRepairs();
                    }, 300);
                }
