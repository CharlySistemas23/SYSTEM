...
router.get('/', requireBranchAccess, async (req, res) => {
  try {
    ...
    res.status(error?.status || 500).json({ error: error?.message || 'Error al obtener inventario' });
  }
});
...
router.get('/:id', requireBranchAccess, async (req, res) => {
  try {
    ...
    res.status(error?.status || 500).json({ error: error?.message || 'Error al obtener item' });
  }
});
...
router.post('/', requireBranchAccess, async (req, res) => {
  ...
  try {
    ...
    res.status(error?.status || 500).json({ error: error?.message || 'Error al crear item' });
  } finally {
    client.release();
  }
});
...
router.delete('/:id', requireBranchAccess, async (req, res) => {
  ...
  try {
    ...
    res.status(error?.status || 500).json({ error: error?.message || errorMessage });
  } finally {
    client.release();
  }
});
