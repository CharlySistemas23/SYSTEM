...
import { query as dbQuery, getDatabaseDiagnostics } from './config/database.js';

// Configurar dotenv
dotenv.config();

const toAsciiSafeText = (value) => {
...
const sanitizeConsoleArg = (arg) => {
  if (typeof arg === 'string') {
    return toAsciiSafeText(arg);
  }

  if (arg instanceof Error) {
    return {
      name: toAsciiSafeText(arg.name),
      message: toAsciiSafeText(arg.message),
      stack: toAsciiSafeText(arg.stack || '')
    };
  }
...
const installSafeConsoleOutput = () => {
  if (globalThis.__SAFE_CONSOLE_OUTPUT_INSTALLED__) return;
  ...
  globalThis.__SAFE_CONSOLE_OUTPUT_INSTALLED__ = true;
};

installSafeConsoleOutput();
...
const parseConfiguredOrigins = () => {
  ...
  const allowAll = merged.length === 0 || merged.includes('*');
  return { allowAll, origins: merged };
};
...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    console.warn(`[CORS] Origen rechazado: ${origin}. Permitidos: ${origins.join(', ')}`);
...
app.get('/health', (req, res) => {
  ...
  res.json({ 
    ...
    dbHost,
    dbDiagnostics: getDatabaseDiagnostics()
  });
});
...
import { setIO as setCashIO } from './routes/cash.js';
import { setIO as setEmployeesIO } from './routes/employees.js';
setInventoryIO(io);
setSalesIO(io);
setBranchesIO(io);
setRepairsIO(io);
